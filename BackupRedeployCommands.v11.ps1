Clear-Host
<## ----- About: ----
    # Backup Redeploy Commands Generator
    # Revision v11 - 2020-08-14
...
# ----- Authenticate ----
    ...
    Write-Host "  Enter Your Login Credentials"
    Write-Host ""
    Write-Host "  Current PartnerName: " $AdminLoginPartnerName
    Write-Host "  Current UserName:    " $AdminLoginUserName
    Write-Host $Global:strLineSeparator
...
# ----- End Authenticate ----
    ...
    $csvfile = "$PSScriptRoot\BMoutput.csv"
    $csvcommands = "$PSScriptRoot\BMCommands.csv"
    #$array = @(Import-CSV "$csvfile")
    $url = "http://cdn.cloudbackup.management/maxdownloads/mxb-windows-x86_x64.exe"
    $download = 'Invoke-WebRequest http://cdn.cloudbackup.management/maxdownloads/mxb-windows-x86_x64.exe -outfile $env:temp\mxb-windows-x86_x64.exe'
    ...
    $check = '(Start-Sleep -s 120) | & "c:\program files\backup manager\clienttool.exe" control.status.get'
    $installer = '& $env:TEMP\mxb-windows-x86_x64.exe'

    $BackupFilter = "at == 1"
    #$BackupFilter = "AA843 != 'VSA' AND AA77 != ''"
    $CountToLookup = 600
    ...
    Write-output "Partnername`tOStype`tid`tcomputername`tdevicename`tdownload`tcommand`tcheck" | Out-file $csvcommands
...
# (Call the JSON Web Request Function to get the GetPassphrase Object)
        ...
        Write-output ("$partnername`t$ostype`t$id`t$Computername`t$devicename`t$downloadold`t" + '& c:\windows\temp\' + "mxb-windows-x86_x64.exe -silent -user `"$DeviceName`" -password `"$password`" -passphrase `"$passphrase`"`t$check ") | Out-file $csvcommands -append
    }
       
        #add-content -path $csvcommands -value '213123,name,command'

Write-output $Global:strLineSeparator
Write-output "  Redeployment Credential Files found here:"
Write-output $Global:strLineSeparator
