<# ----- About: ----
    # Get N-able Backup Session File Details
    # Revision v22 - 2021-01-28
...
<# ----- Behavior: ----
    ...
    # Use the -Action "VHDinFS" parameter to get a list of VHD files that were selected/ protected during the most recent FileSystem backup session
    # Use the -DeviceCount ## (default=20) parameter to define the maximum number of devices to process
    # Use the -FileCount ## (default=10) parameter to define how many files to return per device
...
[CmdletBinding()]
    Param (
        [Parameter(Mandatory=$False)] 
            [ValidateSet('FSSent','FSSize','ESXCount','HVCount','HDinFS')] $Action = "FSSent",      ## Change default Action to (FSSent,FSSize,ESXCount,HVCount,VHDinFS)
...
#region ----- Environment, Variables, Names and Paths ----
    Clear-Host
    $ConsoleTitle = "Get Session File Details for | $Action"
...
Function Set-APICredentials {
    ...
    $BackupCred = Get-Credential -UserName "" -Message 'Enter Login UserName or Email and Password for N-able Backup.Management API'
...
Function CallJSON($url,$object) {
    ...
    Function Get-Devices {
        ...
        ForEach ( $DeviceResult in $Devices.result.result ) {
            ...
            if (($Script:Session.count -ge 1) -and($sessionId)) {
               ...
               $Script:loopcounter = $filecount /5

                do{

                    Get-ProcessedFiles $DeviceResult.AccountId
                    [int]$FileShift = [int]$FileShift + 5
                    Write-Host -NoNewline "$Script:loopcounter "
                    
                    $Script:loopcounter = $Script:loopcounter -1
                 }
                 while($Script:loopcounter -gt 0)
    ...
    Function Get-SessionIds($DeviceId) {
        ...
        $Script:SessionIdResponse = Invoke-RestMethod @params 

        if ($SessionIdResponse.error) {
            Visa-Check
        }
        else{$response = [string]$SessionIdResponse -replace("[][]","") -replace("[{}}]","") -replace('    ','') -replace("`n`n`n","") -replace("\+","") -replace('"SessionId" : ','') -split("`n,")
    ...
    Function Get-ProcessedFiles($DeviceId) {
        ...
        if ($Script:ProcessedFilesResponse.error) {
            $Script:ProcessedFilesResponse = $null
    ...
    Function Get-ProcessedFiles_new($DeviceId) {
        ...
        $Script:ProcessedFilesResponse = Invoke-RestMethod @params 

        #$url3

        if ($ProcessedFilesResponse.error) {
            $Script:ProcessedFilesResponse = $null
            $script:loopcounter = 0
        }else{
