...
<# ----- Behavior: ----
    ...
    # Use the -Delimiter (default=',') parameter to set the delimiter for XLS/CSV import/export (i.e. use ';' for The Netherland)
...
[CmdletBinding(DefaultParameterSetName="Export")]
    Param (
        [Parameter(ParameterSetName="Export",Mandatory=$False)] [switch]$AllPartners,                       ## $AllPartners = $true, to Skip GUI partner selection
        [Parameter(ParameterSetName="Export",Mandatory=$False)] [switch]$AllDevices,                        ## $AllDevices = $true, to Skip GUI device selection
        ...
        [Parameter(ParameterSetName="Import",Mandatory=$False)] [switch]$Import,                            ## 
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function GetM365Stats {
        ...
        $script:devicestatistics = $M365response.deviceStatistics | Select-object @{N="Partner";E={$device.partnername}},
                                                                                    ...
                                                                                    @{N="MailBox";E={$_.datasources.status[0] }},
                                                                                    @{N="OneDrive";E={$_.datasources.status[1] }},
                                                                                    @{N="SharePoint";E={$_.datasources.status[2] }},
    ...
    Function Join-Reports {
        ...
        $Script:M365Users = Join-Object -left $script:M365UserStatistics -LeftJoinProperty UserGuid -right $script:devicestatistics -RightJoinProperty UserGuid -Prefix 'Ext_' -Type AllInLeft -RightProperties SharePoint
        ...
        'Export' { 
            ...
            foreach ($device in $SelectedDevices) {
                ...
                If ($Script:Exportcombined) {
                    ...
                    $Script:DeviceStatistics | Select-object * | Export-CSV -path "$csvoutputfile2a" -delimiter "$Delimiter" -NoTypeInformation -Encoding UTF8 -append
                }

                If ($Script:Exportcombined) {
                    $Script:csvoutputfile2b = "$ExportPath\$($CurrentDate)_M365_User_Selection_Export_$($Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($PartnerId).csv"
                    $Script:M365UserStatistics | Select-object * | Export-CSV -path "$csvoutputfile2b" -delimiter "$Delimiter" -NoTypeInformation -Encoding UTF8 -append
                }
                ...
                If ($Script:ExportIndividual) {
                    ...
                    $Script:DeviceStatistics | Select-object * | Export-CSV -path "$csvoutputfile3a" -delimiter "$Delimiter" -NoTypeInformation -Encoding UTF8 -append
                }     
                
                If ($Script:ExportIndividual) {
                    $Script:csvoutputfile3b = "$ExportPath\$($CurrentDate)_M365_User_Selection_Export_$($device.Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($device.PartnerId).csv"
                    $Script:M365UserStatistics | Select-object * | Export-CSV -path "$csvoutputfile3b" -delimiter "$Delimiter" -NoTypeInformation -Encoding UTF8 -append
                }     
