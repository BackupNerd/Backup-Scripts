...
Function Get-DownloadToken ([string]$FileId) {
    ...
    try {
        ...
        Write-Warning "  download-tokens POST failed for file $FileId : $($_.Exception.Message)"
        return $null
    }
...
Function Get-NotificationsSession {
    ...
    try {
        ...
        Write-Warning "  negotiate failed HTTP $sc — generate will proceed without sessionId"
        return $null
    }
