<# ----- About: ----
    # N-able Backup Add Notifier User
    # Revision v05 - 2021-06-17
...
<# ----- Behavior: ----
    ...
    # The Notifier level user roleid is selectable only from within the API
    # NonInteractive, Notifier level users do not have SSO login access to https://backup.management
    #
    # Note: Notifier users will not show up under the Users list in https://backup.management 
    # Notifier users are visible only as To: recipients in the Scheduled Email views section
    # Notifier users and the Email View to be schedules must both exist at the same partner level
    # 
    ...
    # Use the -Add switch parameter to Add a New Notifier level user
    # Use the -List switch parameter to List current Notifier level users
    # Use the -Remove switch parameter to Remove a current Notifier level user
...
#region ----- Environment, Variables, Names and Paths ----
    ...
    Write-output "  -Sub Partners  = $SubPartners"    
