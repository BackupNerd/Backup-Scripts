...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)][int]$PhysServerFUGB = 2000,                                       ## Selected Size Fare Use GB per Physical Server
        [Parameter(Mandatory=$False)][int]$VirtServerFUGB = 2000,                                       ## Selected Size Fare Use GB per Virtual Server
        [Parameter(Mandatory=$False)][int]$WorkstationFUGB = 500,                                       ## Selected Size Fare Use GB per Workstation
        [Parameter(Mandatory=$False)][Switch]$SendToCWM = $false,                                                ## Send Usage Data to ConnectWise Manage
        [Parameter(Mandatory=$False)][Switch]$DebugCWM = $false,                                                 ## Enable Debug for ConnectWise Manage
...
#region ----- Environment, Variables, Names and Paths ----
    ...
    $Cove2PSAProductMapping = @{
        ...
        "PhysServerQty"     = "Cove Physical Servers"   ## Prefered = 'Cove Physical Servers'
        "VirtServerQty"     = "Cove Virtual Servers"    ## Prefered = 'Cove Virtual Servers'
        "WorkstationQty"    = "Cove Workstations"       ## Prefered = 'Cove Workstations'
        "DocumentsQty"      = "Cove Documents"          ## Prefered = 'Cove Documents'
        "M365UserQty"       = "Cove M365 Users"         ## Prefered = 'Cove M365 Users'
        "ContinuityQty"     = "Cove Continuty"          ## Prefered = 'Cove Continuity'
    ...
    Function Send-EnumeratePartners {
        ...
        # Break Script
            ...
            if ($AllPartners) {
                ...
                $Script:Selection = $Script:SelectedPartners |Select-object id,Name,Level,CreationTime,State,TrialRegistrationTime,TrialExpirationTime,Uid,@{l='VatNumber';e={$_.Company.VatNumber}} | sort-object Level,name | out-gridview -Title "Current Partner | $partnername" -OutputMode Single
...
#region ----- ConnectWise Manage Body / Functions  ----
    if ($SendtoCWM) {
        ...
        Function LookupCWMAgreements {
            ...
            $Agreements = Get-CWMAgreement -condition "name=`"$CWMAgreementName`"" -all                              ## Agreement Name matches
            
            # $Agreements = Get-CWMAgreement -condition "name like `"%$CWMAgreementName%`"" -all                      ## Agreement Name contains
            # $Agreements = Get-CWMAgreement -condition "name like `"$CWMAgreementName%`"" -all                       ## Agreement Name begins with
            # $Agreements = Get-CWMAgreement -condition "name like `"%$CWMAgreementName`"" -all                       ## Agreement Name ends with   
