...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)][switch]$AllPartners = $false,                                     ## Skip GUI partner selection
        [Parameter(Mandatory=$False)][int]$DeviceCount = 15000,                                         ## Change Maximum Number of current devices results to return
        [Parameter(Mandatory=$False)][Switch]$DebugCDP = $false,                                        ## Enable Debug for Cove Max Value / Device Statistics
    ...
    Function Send-EnumeratePartners {
        ...
        # Break Script
            ...
            if ($AllPartners) {
                ...
                [array]$Script:Selection = $Script:SelectedPartners |Select-object id,Name,Level,CreationTime,State,TrialRegistrationTime,TrialExpirationTime,Uid,@{l='VatNumber';e={$_.Company.VatNumber}} | sort-object Level,name | out-gridview -Title "Current Partner | $partnername" -OutputMode Multiple
   ...
   Function Join-Reports {
    ...
    try {
        ...
        if($null -eq $Selection) {
            ...
            Write-Output "`nPulling data for $Partnername`n"
        }
