<## ----- About: ----
    # Set GUI Password via Send Remote Command - 
    # Revision v07 - 2020-11-10
...
<# ----- Behavior: ----
    ...
    #
    # Use -SetGUIPassword parameter (DEFAULT) to be prompted to enter a Secure GUI Password to set
    # Use -RestoreOnly parameter with -SetGUIPassword to allow restores when GUI Password is set
    # Use -WipeGUIPassword parameter to remove current GUI Password
    #
    # Use -AllPartners parameter to Autoselect All Child Partners
    # Use -AllDevices parameter to Autoselect All Devices under Selected Partner/s
    #
    ...
    # Enmerate Devices / Select Devices
    # Prompt for and Set GUI Password 
    # or Wipe existing GUI Password
    #
...
[CmdletBinding(DefaultParameterSetName="SetGUIPW")]
    Param (
        ...
        [Parameter(ParameterSetName="SetGUIPW",Mandatory=$False)] 
            [Parameter(ParameterSetName="WipeGUIPW",Mandatory=$False)][Switch]$AllPartners,   ## Apply to All Child Partners  
        [Parameter(ParameterSetName="SetGUIPW",Mandatory=$False)] 
            [Parameter(ParameterSetName="WipeGUIPW",Mandatory=$False)] [Switch]$AllDevices,   ## Apply to All Devices under Selected Partner              
        [Parameter(Mandatory=$False)] [Alias("Clear")] [switch]$ClearCredentials   ## Remove Stored API Credentials at start of script
        
     )   

    Clear-Host
    Write-output "  Set GUI Password"
    Write-output ""
...
Function CallJSON($url,$object) {
    ...
    Function Send-RemoteCommand { 
        ...
        if ($SecurePassword.length -ge 1) {$UnsecureGUIPassword = [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($SecurePassword))}
        if ($RestoreOnly) {
        ...
        if ($WipeGUIPassword) {
            $UnsecureGUIPassword = ""
            Write-Output    "  Wiping GUI Password for $($SelectedDevices.AccountId.count) Devices, please be patience. "
        }
    ...
    if($null -eq $SelectedDevices) {
        ...
        if ($PSCmdlet.ParameterSetName -eq "SetGUIPW") {
            ...
            Write-Output    $Script:strLineSeparator
            Write-Output    "  Applying GUI Password to $($SelectedDevices.AccountId.count) Devices, please be patience. "
            }
