...
<# ----- Behavior: ----
    ...
    # Use the -Action "HVCount" Parameter to Get a Hyper-V VMs selected / protected during the most recent Hyper-V backup session
...
[CmdletBinding()]
    Param (
        [Parameter(Mandatory=$False)] $Action = "FSSentSize",                       ## Change default Action to FSSentSize,HVCount,VHDinFS 
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function Get-ProcessedFiles($DeviceId) {
        ...
        $Script:csvoutputfile = "$PSScriptRoot\$($CurrentDate)_$($action)_Files_$($Partnername -replace(`" \(.*\)`",`"`") -replace(`"\*`",`"`"))_$($PartnerId).csv"

        $Script:cleanresponse | Select-object * | Export-CSV -path "$csvoutputfile" -delimiter "$Delimiter" -NoTypeInformation -Encoding UTF8 -append 
    }
