...
[CmdletBinding(DefaultParameterSetName="ListUsers")]
    Param (
        [Parameter(ParameterSetName="AddUsers",Mandatory=$False)] [Switch]$Add,                     ## Allow Restore Only GUI Access
        [Parameter(ParameterSetName="ListUsers",Mandatory=$False)] [Switch]$List,                   ## Clear GUI Password   
        [Parameter(ParameterSetName="RemoveUsers",Mandatory=$False)] [switch]$Remove,               ## Specify GUI Password to set
        [Parameter(ParameterSetName="ListUsers",Mandatory=$False)] 
            [Parameter(ParameterSetName="RemoveUsers",Mandatory=$False)][Switch]$AllPartners,       ## Skip partner selection
        ...
        [Parameter(ParameterSetName="ListUsers",Mandatory=$False)] [switch]$ClearCredentials        ## Remove Stored API Credentials at start of script
        
    )   

#region ----- Environment, Variables, Names and Paths ----
    Clear-Host
    Write-output "  Add Notifier User`n`n"
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function Send-AddUser { 
        ...
        Write-Output "  Enter details to create a new 'Notifier User' for Scheduled Email delivery, without Backup Console access."
        Write-Output ""
        ...
        $data.params.userInfo.Title        = "User Created by $($Script:Authenticate.result.result.EmailAddress)"
        ...
        Write-Output "  New User $($Script:NewUser.result.result) Added"    
    }
    ...
    switch ($PsCmdlet.ParameterSetName)
    {
        "AddUsers" {
            ...
            Send-EnumerateUsers
            $Script:EnumerateUsers.result.result | Select-Object Partnerid,Name,emailAddress,FullName,id,Roleid,Title,Flags,PhoneNumber | Where-object {$_.RoleId -eq 7} | Format-Table
            If ($Script:EnumerateUsers.result.result -eq $null) {Write-Output "  Zero Notifier Users Found" }
            Send-AddUser
            Send-ModifyUser
            Send-EnumerateUsers
            $Script:EnumerateUsers.result.result | Select-Object Partnerid,Name,emailAddress,FullName,id,Roleid,Title,Flags,PhoneNumber | Where-object {$_.RoleId -eq 7} | Format-Table
        ...
        "ListUsers" {
            Send-EnumeratePartners 
            Send-EnumerateUsers
            $Script:EnumerateUsers.result.result | Select-Object Partnerid,Name,emailAddress,FullName,Roleid,id,Title,Flags,PhoneNumber | Where-object {$_.RoleId -eq 7} | Format-Table
            If (($Script:EnumerateUsers.result.result -eq $null) -or ($Script:EnumerateUsers.result.result -eq 0 )) {Write-Output "  Zero Notifier Users Found" }
        }
        "RemoveUsers" {
            Send-EnumeratePartners 
            Send-EnumerateUsers
            $Script:SelectedUser = $Script:EnumerateUsers.result.result | Select-Object Partnerid,Name,emailAddress,FullName,id,Roleid,Title,Flags,PhoneNumber | Where-object {$_.RoleId -eq 7} | Out-GridView -OutputMode Single
            If (($Script:SelectedUser -eq $null) -or ($Script:SelectedUser -eq 0)) {Write-Output "  No Notifier User Found/ Selected" }
