...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)][int]$DeviceCount = 500,                                    ##  Maximum Device Count to Return
        ...
        [Parameter(Mandatory=$False)][switch]$ExcludeDocuments = $false,                         ##  set switch to $true to exclude documents devices from deletion      
        [Parameter(Mandatory=$False)][switch]$ExcludeWorkstations = $false,                      ##  set switch to $true to exclude workstations from deletion
        ...
        [Parameter(Mandatory=$False)][Switch]$ForceDelete = $false                              ##  Set switch to $true to force deletion without confirmation
...
$DeviceFilter = "$($arex)( (AT == 1)$($unex)$($wsex) AND (OT != 2) $($DOex) ) AND ( ( TS <= $InactiveTimeStamp.days().ago() ) OR ( TS <= $UnusedTimeStamp.days().ago() AND US <= $UsedStorageLessThanBytes AND T3 <= $SelectedSizeLessThanBytes ) )"
