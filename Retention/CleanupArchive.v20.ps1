...
<# ----- Compatibility: ----
    ...
    # For use with N-able RMM and N-central integrated Backup editions
...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)] [switch]$Force,                               ## Do not prompt for confirmation before Archive Cleaning
...
# (Create the JSON object to call the CleanupArchiving function)
    ...
    Add-Member -PassThru NoteProperty visa $script:BackupFPvisa |
