...
<# ----- Behavior: ----
    ...
    #
    # User the -OneDriveAge ## days (Default=45) parameter to define the age in days of shared billable users to identify
    ...
    #
    # User the -DeletedAge ## days (Default=45) parameter to define the age in days of deleted users to identify
    ...
    if ($ExportSharedBillable) {
        ...
        if ($CleanupSharedBillable) {
            ...
            foreach ($item in $OneDriveToClean) {
                visa-check
                Write-output "Attempting to Deselect OneDrive from Shared Mailbox"
    ...
    if ($ExportDeleted) {
        ...
        if ($CleanupDeleted) {
            ...
            foreach ($item in $DeletedToClean) {
                visa-check
                Write-output "Attempting to Deselect OneDrive from Deleted Mailbox"
