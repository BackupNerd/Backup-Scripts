...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)] [switch]$AllDevices = $false,                     ## Skip device selection
        ...
        [Parameter(Mandatory=$False)] [int]$ThrottleLimit = 10                               ## Max parallel threads (PS 7+ only)
...
Function Send-GetDevices {
    ...
    $data.params.query.Columns = @("AU","AR","AN","TM","MN","AL","LN","OP","OI","OS","OT","PD","AP","PF","PN","CD","TS","TL","T3","US","TB","TM","I81","AA843","AA77","AA2048","AA58","FA","F3","F5","SA","S3","S5","ZA","Z3","Z5")
...
0 != 1 and (
    ...
    Operation == 'execute command: set selection'
)
"@,
    ...
    while (-not $success -and $retryCount -lt $maxRetries) {
        try {
            $Script:DeviceAuditQueryResponse = Invoke-RestMethod -Uri $Script:repsvr.repurl -Method POST -Headers $headers -ContentType "application/json" -Body $body -TimeoutSec 30
            $success = $true
            ...
            if ($_.Exception.Message -like "*underlying connection was closed*" -or 
                ...
                if ($retryCount -lt $maxRetries) {
                    Write-Warning "Connection failed for device $($Script:repsvr.Name)| $($Script:repsvr.AccountId). Retry $retryCount of $maxRetries in $retryDelay seconds..."
                    Start-Sleep -Seconds $retryDelay
                    $retryDelay = $retryDelay * 2  # Exponential backoff
                }
                else {
                    Write-Error "Failed to get audit query for device $($Script:repsvr.Name)| $($Script:repsvr.AccountId) after $maxRetries retries: $($_.Exception.Message)"
                ...
                Write-Error "Failed to get audit query for device $($Script:repsvr.Name)| $($Script:repsvr.AccountId): $($_.Exception.Message)"
                return
            }
        }
    }
...
if($null -eq $SelectedDevices) {
    ...
    If ($Script:Export) {
       ...
       # $Script:csvoutputfile1 = "$ExportPath\$($CurrentDate)_InitialSessions_$($Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($PartnerId).csv"
    ...
    if ($PSVersionTable.PSVersion.Major -ge 7) {
        ...
        $Script:SelectedDevices | ForEach-Object -Parallel {
            ...
            # Batch update all 4 custom columns in a single API call
            $batchSuccess = Send-BatchUpdateCustomColumns -DeviceId $device.AccountID -Updates @(
                @(2649, $filterString),
            ...
            if ($batchSuccess) {
                Write-Output "  Updated 4 columns | $($device.DeviceName)"
            } else {
        ...
        if ($failedBag.Count -gt 0) {
            ...
            foreach ($device in $failedBag) {
                ...
                if ($batchSuccess) { Write-Output "  Second pass: updated 4 columns | $($device.DeviceName)" }
        ...
        foreach ($device in $Script:SelectedDevices) {
            ...
            if ($batchSuccess) {
                Write-Output "  Successfully updated all 4 custom columns in batch"
            } else {
...
} ## End Generate Statistics XLS from CSV

if ($csvoutputfile1) {
    $xlsoutputfile1 = $csvoutputfile1.Replace("csv","xlsx")
    Save-CSVasExcel $csvoutputfile1
}  ## End Generate Session XLS from CSV
...
if ($Launch) {
    If (test-path HKLM:SOFTWARE\Classes\Excel.Application) { 
        ...
        Start-Process "$xlsoutputfile1"
        Write-output $Script:strLineSeparator
        Write-Output "  Opening XLS file"
        }else{
        Start-Process "$csvoutputfile"
        Start-Process "$csvoutputfile1"
        Write-output $Script:strLineSeparator
...
Write-Output "  CSV Path = $Script:csvoutputfile1"
Write-Output "  XLS Path = $Script:xlsoutputfile1"
