<# ----- About: ----
    # N-able Backup Bulk Set Device Product 
    # Revision v18 - 2021-10-20
...
#region ----- Environment, Variables, Names and Paths ----
    Clear-Host
    Write-Output "  Set Device Product`n`n"
...
# (No error)
        ...
        $Script:SelectedPartner = $EnumeratePartnersSessionResults | Sort-object Level,Name | Select-object Name,Id,Level,Externalcode |
            ...
            Out-Gridview -Title "Select a PARTNER from available PARTNERS to list available PRODUCTS" -OutputMode Single
...
# (Create the JSON object to call the EnumerateProducts function)
    $objEnumerateProducts = (New-Object PSObject | Add-Member -PassThru NoteProperty jsonrpc ‘2.0’ |
        Add-Member -PassThru NoteProperty visa $Script:visa |
        Add-Member -PassThru NoteProperty method ‘EnumerateProducts’ |
        Add-Member -PassThru NoteProperty params @{partnerId=[int]$SelectedPartner.id } |
        Add-Member -PassThru NoteProperty id ‘1’)| ConvertTo-Json -Depth 3

# (Call the JSON Web Request Function to get the EnumerateProducts Object)
    [array]$EnumerateProductsSession = CallJSON $urlJSON $objEnumerateProducts
...
# (Get Result Status of EnumerateProducts)
    $EnumerateProductsSessionErrorCode = $EnumerateProductsSession.error.code
    $EnumerateProductsSessionErrorMsg = $EnumerateProductsSession.error.message

# (Check for Errors with EnumerateProducts - Check if ErrorCode has a value)
    if ($EnumerateProductsSessionErrorCode) {
        Write-Output    $Script:strLineSeparator
        Write-Output    "  EnumerateProductsSession Error Code:  $EnumerateProductsSessionErrorCode"
        Write-Output    "  EnumerateProductsSession Message:  $EnumerateProductsSessionErrorMsg"
        Write-Output    $Script:strLineSeparator
        Write-Output    "  Exiting Script"
# (Exit Script if there is a problem)
...
# (No error)
        ...
        $Script:EnumerateProductsSessionResults = $EnumerateProductsSession.result.result | select-object Name,@{l='Id';e={($_.Id).tostring()}} | where-object {$_.name -ne "Documents"} | Sort-Object Name

        $Script:SelectedProduct = $EnumerateProductsSessionResults | where-object {$_.Name -ne "Documents"} | select-object Name,@{l='Id';e={($_.Id).tostring()}} |
            Out-Gridview -Title "Select a PRODUCT from available PRODUCTS to later apply to selected DEVICES" -OutputMode Single | Sort-Object Name,Id

        Write-Output "$(Get-LogTimeStamp) Selected Product = $($SelectedProduct.name) | $($SelectedProduct.id)" | Out-file $ScriptLog -append

        if ($SelectedProduct.id -eq $null) {Write-output "  Script Exited, No Changes Made`n"; exit}

        Write-Output    $Script:strLineSeparator
        Write-Output    "  Selected Product = $($SelectedProduct.name)"
        Write-Output    $Script:strLineSeparator
...
# (Summarize DeviceDetail)
    ...
    $Script:SelectedDevice = $DeviceDetail | Select-Object PartnerId,Reference,PartnerName,DeviceName,MachineName,AccountID,Product,ProductID,AccountType,Profile,OS,OsType,TimeStamp,Notes |
        Out-Gridview -Title "Assign PRODUCT | $($selectedProduct.name) | to selected DEVICES | use SHIFT to select multiple DEVICES" -OutputMode Multiple
...
# (Check for Errors with ModifyAccountSession - Check if ErrorCode has a value)
            if ($ModifyAccountSessionErrorCode) {
                ...
                Write-Output    $Script:strLineSeparator
                Write-Output "$(Get-LogTimeStamp) | DEVICE | $($Selection.AccountID) | $($Selection.DeviceName) | ASSIGN PRODUCT ERROR | $ModifyAccountSessionErrorMsg" | Out-file $ScriptLog -append
...
# (Exit Script if there is a problem)
            ...
            Else {
# (No error)
                Write-Output    "  Product | $($SelectedProduct.Id) | $($SelectedProduct.Name) | Assigned to Device | $($Selection.AccountID) | $($Selection.DeviceName) | Old Product | $($Selection.ProductId) | $($Selection.Product)"    
                Write-Output "$(Get-LogTimeStamp) Product | $($SelectedProduct.Id) | $($SelectedProduct.Name) | Assigned to Device | $($Selection.AccountID) | $($Selection.DeviceName) | Old Product | $($Selection.ProductId) | $($Selection.Product)" | Out-file $ScriptLog -append
  ...
  Function Exit-Routine {
    Write-Output $Script:strLineSeparator
    Write-Output "  NOTE: Product changes are registered immediately with the cloud service, offline and running devices"
    Write-Output "        should reflect the new Product once they reconnect, running jobs complete, or servers check-in."
