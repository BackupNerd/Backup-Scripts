...
Function Write-ExecutionLog {
    ...
    $logEntry = "$timestamp | $Operation | $Email | $UserGuid | $Result | $Details"
...
foreach ($device in $SelectedDevices) {
    ...
    # Display summary unless CDPDebug is enabled
    if (-not $CDPDebug) {
        $totalUsers = $Script:M365UserStatistics.Count
        $billableUsers = ($Script:M365UserStatistics | Where-Object { $_.Protected_Billable -eq 'Billable' }).Count
        ...
        Write-Host "  Total: $totalUsers | Billable: $billableUsers | Shared: $sharedMailboxes | Deleted: $deletedUsers" -ForegroundColor Gray
    }
...
if ($ExportDeleted) {
    ...
    if ($CleanupDeletedBillable) {
        ...
        if ($ForceClean) {
            ...
            $Script:DeletedToClean = $Deleted | Where-Object {($_.Protected_Billable -eq "Billable") -and ($_.Shared -ne "Shared")}
        } else {
    ...
    if ($CleanupDeletedBillableShared) {
        ...
        if ($ForceClean) {
            ...
            $Script:DeletedToClean = $Deleted | Where-Object {($_.Protected_Billable -eq "Billable") -and ($_.Shared -eq "Shared")}
        } else {
    ...
    if ($CleanupDeletedShared) {
        ...
        if ($ForceClean) {
            ...
            $Script:DeletedToClean = $Deleted | Where-Object {($_.Protected_Billable -ne "Billable") -and ($_.Shared -eq "Shared")}
        } else {
