...
<# ----- Behavior: ----
    ...
    #
    # Use the -OneDriveAge ## days (Default=90) parameter to define the age in days of shared billable users to identify
    ...
    #
    # Use the -DeletedAge ## days (Default=180) parameter to define the age in days of deleted users to identify
    # Set the -ExportDeleted switch parameter to $true to identify deleted users
    # Set the -CleanupDeleted switch parameter to $true to prompt for removal of deleted users
    #
...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)] [switch]$CleanupDeleted = $false,           ## Prompt to Cleanup Deleted Mailboxes
...
Function ExitRoutine {
    ...
    if ($AllDevices) {
        $script:SelectedDevices = $DeviceDetail | Select-Object PartnerId,PartnerName,Reference,AccountID,DeviceName,SelectedGB,UsedGB,Last28Days,Errors,Billable,MailBoxes,Shared,OneDrive,SPusers,SPsites,StorageLocation,TimeStamp,LastSuccess,Creation,AccountToken,Notes
    }else{
        $script:SelectedDevices = $DeviceDetail | Select-Object PartnerId,PartnerName,Reference,AccountID,DeviceName,SelectedGB,UsedGB,Last28Days,Errors,Billable,MailBoxes,Shared,OneDrive,SPusers,SPsites,StorageLocation,TimeStamp,LastSuccess,Creation,AccountToken,Notes  | Out-GridView -title "Current Partner | $partnername" -OutputMode Multiple}
        Visa-Check
    if ($null -eq $SelectedDevices) {
        ...
        # OK was pressed, $Selection contains what was chosen
        # Run OK script
        $DeviceDetail | Select-Object PartnerId,PartnerName,Reference,AccountID,DeviceName,SelectedGB,UsedGB,Last28,Errors,Billable,MailBoxes,Shared,OneDrive,SPusers,SPsites,StorageLocation,TimeStamp,LastSuccess,Creation,Notes  | Sort-object PartnerName,AccountId | format-table
    ...
    if ($ExportDeleted) {
        ...
        $Deleted = $Deleted |  Where-Object {$_.MailBoxLastBackupTimestamp} 
