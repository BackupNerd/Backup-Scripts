...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)] [switch]$ExportSharedBillable = $true,                    ## Export Billable Shared Mailboxes with Onedrive History list
        ...
        [Parameter(Mandatory=$False)] [int]$OneDriveAge = 10,                                   ## Ignore Billable Shared Mailboxes with OneDrive Backups in the last X Days 

        [Parameter(Mandatory=$False)] [switch]$ExportUnlicensed = $true,                        ## Export Unlicensed Mailboxes list
        [Parameter(Mandatory=$False)] [switch]$CleanupUnlicensed = $false,                       ## Prompt to Cleanup Deleted Mailboxes
        [Parameter(Mandatory=$False)] [int]$UnlicensedAge = 30,                                 ## Ignore Deleted Mailboxes where in the last X Days

        [Parameter(Mandatory=$False)] [switch]$ExportDeleted = $true,                           ## Export Deleted Mailboxes list
        [Parameter(Mandatory=$False)] [switch]$CleanupDeletedBillable = $false,                  ## Prompt to Cleanup Deleted Mailboxes
        [Parameter(Mandatory=$False)] [switch]$CleanupDeletedShared = $false,                    ## Prompt to Cleanup Deleted Mailboxes
        [Parameter(Mandatory=$False)] [switch]$CleanupDeletedBillableShared = $false,            ## Prompt to Cleanup Deleted Mailboxes
        [Parameter(Mandatory=$False)] [int]$DeletedAge = 30,                                    ## Ignore Deleted Mailboxes where in the last X Days
