...
<# ----- Behavior: ----
    ...
    # Query current selections and schedules via RCG for each online device
    # Maintain the persistent master inventory across runs to capture configuration changes over time
    # Preserve existing selections and schedules when a device is offline or unreachable
    # Recommended operation: run periodically, ideally every 4 hours throughout the day
    ...
    # Use the -MaxRcgAgeHours ## (default=24) parameter to avoid RCG queries for stale devices
    # Use the -SkipRecentHours ## (default=24) parameter to skip RCG selection queries for
    # devices already marked reachable whose stored Timestamp is recent (0 = disabled)
...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)] [int]$ActiveWithinDays = 90,                       ## Only process devices with heartbeat within N days (0 = all devices)
        [Parameter(Mandatory=$False)] [int]$MaxRcgAgeHours = 4,                        ## Do not query RCG for devices older than this (0 = disabled)
        [Parameter(Mandatory=$False)] [int]$SkipRecentHours = 72,                       ## Skip RCG queries for recently validated reachable devices (0 = disabled)
        [Parameter(Mandatory=$False)] [int]$RetryAuthDelayMs = 500,                     ## Delay/jitter before immediate-retry RCG authentication attempts
...
#region ----- Environment, Variables, Names and Paths ----
    ...
    Write-Output "  -MaxRcgAgeHours = $MaxRcgAgeHours  $(if ($MaxRcgAgeHours -gt 0) { '(stale devices will not be queried via RCG)' } else { '(disabled)' })"
    Write-Output "  -SkipRecentHours = $SkipRecentHours  $(if ($SkipRecentHours -gt 0) { '(recent reachable devices will skip RCG selection queries)' } else { '(disabled)' })"
    Write-Output "  -RetryAuthDelayMs = $RetryAuthDelayMs  (immediate-retry RCG auth delay/jitter)"
...
#region ----- Authentication ----
    ...
    Function Send-APICredentialsCookie {
        ...
        if ($resp.visa) {
            ...
            Write-Host "  Authenticated PartnerId (default GetPartnerTree scope): $Script:AuthPartnerId" -ForegroundColor DarkCyan
        } else {
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function Get-CovePartnerInfoById {
        param([int]$PartnerId)
        $data = @{ jsonrpc='2.0'; id='2'; visa=$Script:visa; method='GetPartnerInfoById'; params=@{ partnerId=[int]$PartnerId } }
        $response = Invoke-RestMethod -Method POST -ContentType 'application/json' -Body (ConvertTo-Json $data -Depth 5) -Uri $urlJSON
        if ($response.error) { throw $response.error.message }
        $info = $response.result.result
        [PSCustomObject]@{ Uid=[string]$info.Uid; Id=[int]$info.Id; Name=[string]$info.Name; Level=[string]$info.Level }
    }

    Function Get-CovePartnerByName {
        param([Parameter(Mandatory=$True)][string]$PartnerName, [int]$PartnerId=$Script:AuthPartnerId)
        $data = @{ jsonrpc='2.0'; id='2'; visa=$Script:visa; method='GetPartnerTree'; params=@{ partnerId=[int]$PartnerId; fields=@(0,1,3,4); childrenLimit=5000; partnerFilter=@{ NamePattern=$PartnerName } } }
        $treeResponse = Invoke-RestMethod -Method POST -ContentType 'application/json' -Body (ConvertTo-Json $data -Depth 6) -Uri $urlJSON
        if ($treeResponse.error) { throw $treeResponse.error.message }
        $root = $treeResponse.result.result
        $restricted = @('Root','Sub-root','Distributor')
        $matches = [System.Collections.Generic.List[object]]::new(); $infoById=@{}
        function Add-Match { param($Node)
            if ($Node.Info -and $Node.Info.Id) { $infoById[[int]$Node.Info.Id]=$Node.Info }
            if ($Node.Info -and $Node.Info.Name -and ($Node.Info.Name -like "*$PartnerName*") -and $Node.Info.Level -notin $restricted) { $matches.Add([PSCustomObject]@{Id=[int]$Node.Info.Id;Name=[string]$Node.Info.Name;Level=[string]$Node.Info.Level;ParentId=[int]$Node.Info.ParentId;Path=''})|Out-Null }
            foreach($child in $Node.Children){ Add-Match $child }
        }
        if($root){Add-Match $root}
        if($matches.Count -eq 0){throw "No partner found matching '$PartnerName'."}
        foreach($m in $matches){$parts=[System.Collections.Generic.List[string]]::new();$node=$infoById[[int]$m.Id];$seen=[System.Collections.Generic.HashSet[int]]::new();while($node -and $seen.Add([int]$node.Id)){$parts.Insert(0,[string]$node.Name);$node=$infoById[[int]$node.ParentId]};$m.Path=$parts -join ' > '}
        if($matches.Count -gt 1){$selected=$matches|Select-Object Id,Name,Level,ParentId,Path|Out-GridView -Title "Select Partner matching '$PartnerName'" -OutputMode Single;if(-not $selected){throw 'No partner selected.'}}else{$selected=$matches[0]}
        Get-CovePartnerInfoById -PartnerId ([int]$selected.Id)
    }
    ...
    Function Send-GetDevices {
        ...
        $data.params.query.Columns = @(
            ...
            "TL",
            ## Storage node/status correlation
            "YS",
        ...
        ForEach ($DeviceResult in $Script:DeviceResponse.result.result) {
            $Script:DeviceDetail += New-Object -TypeName PSObject -Property @{
                ...
                StorageStatus = $DeviceResult.Settings.YS -join ''
    ...
    Function Get-RcgAuthFailureReason {
        param(
            [string]$StatusCode,
        ...
        $headerNamesText = if ($HeaderNames) { @($HeaderNames) -join ', ' } else { '' }
        $cookiePresent = [bool]($SetCookieHeader -match '(?i)(^|;\s*)visa=')
        $bodyText = if ($BodyPreview) { $BodyPreview } else { '' }
        $sanitized = $bodyText -replace '(?i)(auth_token|visa)=([^&;\s]+)', '$1=<redacted>' -replace '\s+', ' ' 

        return "RCG visa missing; HTTP $StatusCode $StatusDescription; ContentType=[$ContentType]; Headers=[$headerNamesText]; SetCookieVisa=$cookiePresent; Content=$sanitized"
    }
    ...
    Function Get-RcgSelections ($AccountId, $DeviceName, $DeviceDataSources, $Visa = $null, $DsMap = $null, $ApMap = $null) {
        ...
        if ($rcgResp.error) {
            ...
            return @{ Selections = $null; ErrorReason = "EnumerateAccountRemoteAccessEndpoints: $($rcgResp.error.message)"; RcgHost=$rcgHost; InstallationId=$installationId; InstallationMode=$installationMode }
        }
        ...
        if (-not $endpointsList -or @($endpointsList).Count -eq 0) {
            ...
            return @{ Selections = $null; ErrorReason = 'No RCG endpoint returned'; RcgHost=$rcgHost; InstallationId=$installationId; InstallationMode=$installationMode }
        }

        $endpoint   = @($endpointsList)[0]
        $rcgUrl     = $endpoint.WebRcgUrl
        $installationId = [string]$endpoint.InstallationId
        $installationMode = [string]$endpoint.InstallationMode
        if ($rcgUrl) { $rcgHost = ([Uri]$rcgUrl).Host }
        ...
        if (-not $rcgUrl) {
            ...
            return @{ Selections = $null; ErrorReason = 'No RCG URL returned'; RcgHost=$rcgHost; InstallationId=$installationId; InstallationMode=$installationMode }
        }
        if (-not ($rcgUrl -match "auth_token=([^&]+)")) {
            ...
            return @{ Selections = $null; ErrorReason = 'No auth_token in WebRcgUrl'; RcgHost=$rcgHost; InstallationId=$installationId; InstallationMode=$installationMode }
        }
        $authToken = $matches[1]
        ...
        try {
            ...
            if (-not $rcgVisa) {
                $headerNames = @($getResp.Headers.Keys)
                $setCookieHeader = @($getResp.Headers['Set-Cookie']) -join ' | '
                $contentType = [string]$getResp.Headers['Content-Type']
                $bodyPreview = if ($getResp.Content) { ([string]$getResp.Content).Trim() } else { '' }
                if ($bodyPreview.Length -gt 250) { $bodyPreview = $bodyPreview.Substring(0,250) + '...' }
                $diagnostic = Get-RcgAuthFailureReason -StatusCode ([int]$getResp.StatusCode) -StatusDescription $getResp.StatusDescription -ContentType $contentType -HeaderNames $headerNames -SetCookieHeader $setCookieHeader -BodyPreview $bodyPreview
                Write-Host "  RCG visa missing (HTTP $([int]$getResp.StatusCode)); detailed response diagnostics will be written to the RCG error log." -ForegroundColor Red
                return @{ Selections = $null; ErrorReason = $diagnostic; RcgHost=$rcgHost; InstallationId=$installationId; InstallationMode=$installationMode }
            }
        } catch {
            Write-Host "  RCG auth GET failed: $($_.Exception.Message)" -ForegroundColor Red
            return @{ Selections = $null; ErrorReason = "RCG auth GET failed: $($_.Exception.Message)"; RcgHost=$rcgHost; InstallationId=$installationId; InstallationMode=$installationMode }
        }
        ...
        $pluginErrorBag = [System.Collections.Concurrent.ConcurrentBag[object]]::new()
        $allSelections = $dsPluginsToQuery | ForEach-Object -Parallel {
            $plugin   = $_
            $rpcUrl   = $using:rcgRpcUrl
            $errorBag = $using:pluginErrorBag
            ...
            while ($attempt -lt $maxRetries -and $null -eq $result) {
                ...
                try {
                    $resp = Invoke-RestMethod -Uri $rpcUrl -Method POST -Headers $headers `
                        -TimeoutSec 30 -Body $payload -ErrorAction Stop
        ...
        ## Stage 3b: schedules use the same retry policy as datasource selections.
        $rawSchedules     = $null
        $rawHFSchedules   = $null
        $schedHeaders = @{ "Content-Type"="application/json"; "Cookie"="visa=$rcgVisa"; "Cache-Control"="no-cache" }
        $scheduleMaxRetries = 3
        $scheduleRetryDelay = 100
        ...
        for ($scheduleAttempt = 1; $scheduleAttempt -le $scheduleMaxRetries -and -not $rawSchedules; $scheduleAttempt++) {
            try {
                $rawSchedules = Invoke-RestMethod -Uri $rcgRpcUrl -Method POST -Headers $schedHeaders `
                    -Body $schedPayload -TimeoutSec 30 -ErrorAction Stop
            } catch {
                if ($scheduleAttempt -eq $scheduleMaxRetries) { $rawSchedules = @{ _error = $_.Exception.Message } }
                else { Start-Sleep -Milliseconds $scheduleRetryDelay }
            }
        }

        for ($scheduleAttempt = 1; $scheduleAttempt -le $scheduleMaxRetries -and -not $rawHFSchedules; $scheduleAttempt++) {
            try {
                $rawHFSchedules = Invoke-RestMethod -Uri $rcgRpcUrl -Method POST -Headers $schedHeaders `
                    -Body $hfPayload -TimeoutSec 30 -ErrorAction Stop
            } catch {
                if ($scheduleAttempt -eq $scheduleMaxRetries) { $rawHFSchedules = @{ _error = $_.Exception.Message } }
                else { Start-Sleep -Milliseconds $scheduleRetryDelay }
            }
        }
        ...
        if ($pluginErrorBag.Count -gt 0) {
            $errorReasons.Add('Datasource plugin failure(s): ' + (($pluginErrorBag | ForEach-Object { "$($_.Plugin) - $($_.Error)" }) -join '; ')) | Out-Null
        }
        if ($rawSchedules._error) { $errorReasons.Add("EnumerateBackupSchedule: $($rawSchedules._error)") | Out-Null }
        if ($rawHFSchedules._error) { $errorReasons.Add("GetHighFrequentBackupSchedule: $($rawHFSchedules._error)") | Out-Null }
        $pluginErrorReason = if ($errorReasons.Count -gt 0) { $errorReasons -join '; ' } else { $null }
    ...
    Function Test-RcgErrorReason {
        param([string]$Reason)

        if ([string]::IsNullOrWhiteSpace($Reason) -or $Reason -eq 'No selections returned' -or $Reason -eq 'Retry returned no selections' -or $Reason -eq 'Final retry returned no selections') {
            return $false
        }

        return $Reason -match '(?i)(RCG|visa missing|endpoint|URL|auth|visa|token|timeout|timed out|connection|transport|SSL|remote access|failed|refused|reset|closed|not respond|not connected|not registered|client is not connected|invalid authentication)' 
    }
...
#region ----- Main Execution ----
    ...
    $selfPartner = Get-CovePartnerInfoById -PartnerId $Script:AuthPartnerId
    $restrictedLevels = @('Root','Sub-root','Distributor')
    if (-not $PartnerName -and $selfPartner.Level -notin $restrictedLevels) { $PartnerName = $selfPartner.Name }
    if (-not $PartnerName) { do { $PartnerName=Read-Host "  Enter Customer/ Partner display name (or partial name)" } while ([string]::IsNullOrWhiteSpace($PartnerName)) }
    $targetPartner = Get-CovePartnerByName -PartnerName $PartnerName -PartnerId $Script:AuthPartnerId
    $Script:Uid=$targetPartner.Uid; $Script:PartnerId=[int]$targetPartner.Id; $Script:PartnerName=$targetPartner.Name; $Script:Level=$targetPartner.Level
    Write-Output $Script:strLineSeparator
    ...
    ## Keep the complete inventory for orphan detection, but skip expensive RCG selection
    ## queries for devices that were reachable recently and already have current master data.
    ## Targeted -FilterAccountIDs and -RetryOnly runs always query their selected devices.
    $allSelectedDevices = @($Script:SelectedDevices)
    $enumeratedIds = @($allSelectedDevices | Select-Object -ExpandProperty AccountID)
    $skippedRecentDevices = [System.Collections.Generic.List[object]]::new()
    $skippedStaleDevices = [System.Collections.Generic.List[object]]::new()
    $devicesToProcess = [System.Collections.Generic.List[object]]::new()

    if (-not $RetryOnly -and $FilterAccountIDs.Count -eq 0 -and ($MaxRcgAgeHours -gt 0 -or $SkipRecentHours -gt 0)) {
        ## First exclude devices whose live API heartbeat Timestamp is too old for a useful RCG attempt.
        if ($MaxRcgAgeHours -gt 0) {
            $staleCutoff = [DateTime]::UtcNow.AddHours(-$MaxRcgAgeHours)
            foreach ($device in $allSelectedDevices) {
                $deviceTimestamp = if ($device.TimeStamp -is [datetime]) { $device.TimeStamp.ToUniversalTime() } else { [DateTime]::MinValue }
                if ($deviceTimestamp -lt $staleCutoff) {
                    $skippedStaleDevices.Add($device) | Out-Null
                } else {
                    $devicesToProcess.Add($device) | Out-Null
                }
            }
            if ($skippedStaleDevices.Count -gt 0) {
                Write-Host "  Skipping $($skippedStaleDevices.Count) device(s) with Timestamp older than $MaxRcgAgeHours hour(s); RCG will not be attempted." -ForegroundColor DarkCyan
            }
        } else {
            foreach ($device in $allSelectedDevices) { $devicesToProcess.Add($device) | Out-Null }
        }

        $recentCutoff = [DateTime]::UtcNow.AddHours(-$SkipRecentHours)
        [string[]]$timestampFormats = @("yyyy-MM-dd HH:mm:ss 'UTC'", 'yyyy-MM-dd HH:mm:ss', 'o')

        if ($SkipRecentHours -gt 0) {
        $recentDevicesToProcess = [System.Collections.Generic.List[object]]::new()
        foreach ($device in @($devicesToProcess)) {
            $masterRow = $masterHash[[string]$device.AccountID]
            $storedTimestamp = [DateTime]::MinValue
            $timestampText = if ($masterRow) { [string]$masterRow.TimeStamp } else { '' }
            $hasRecentTimestamp = [DateTime]::TryParseExact(
                $timestampText,
                $timestampFormats,
            ...
            if ($masterRow -and $masterRow.ReachableStatus -eq 'Yes' -and $hasRecentTimestamp -and $storedTimestamp -ge $recentCutoff -and $storedTimestamp -le [DateTime]::UtcNow) {
                $skippedRecentDevices.Add($device) | Out-Null
            } else {
                $recentDevicesToProcess.Add($device) | Out-Null
            }
        }
        $devicesToProcess = $recentDevicesToProcess
        }

        $Script:SelectedDevices = @($devicesToProcess)
        if ($skippedRecentDevices.Count -gt 0) {
            Write-Host "  Skipping $($skippedRecentDevices.Count) reachable device(s) with Timestamp within the last $SkipRecentHours hour(s); existing master selections will be retained." -ForegroundColor DarkCyan
        }
    }

    ## Capture list of enumerated AccountIDs for orphan detection
    Write-Output "  Enumerated $($enumeratedIds.Count) device(s) for this run; querying $($Script:SelectedDevices.Count) device(s) via RCG"

    Write-Output $Script:strLineSeparator
    Write-Output "  Processing $($Script:SelectedDevices.Count) device(s) | Throttle: $DeviceThrottle | Automatic retry: enabled"
    Write-Output $Script:strLineSeparator
    ...
    $rcgErrorMap  = @{}  ## AccountID -> final unrecovered RCG error with device statistics
    ...
    $rcgAuthFuncText  = ${function:Get-RcgAuthFailureReason}.ToString()
    ...
    $completedFirstPass = 0
    Write-Progress -Id 1 -Activity "RCG Selections — $Script:PartnerName" -Status "[0/$totalDevices] starting..." -PercentComplete 0
    $Script:SelectedDevices | ForEach-Object -Parallel {
        $device = $_

        ## Re-import helper functions into this runspace
        New-Item -Path 'function:\Get-RcgAuthFailureReason' `
            -Value ([scriptblock]::Create($using:rcgAuthFuncText)) -Force | Out-Null
        ...
        try {
            ...
            Write-Host "  [!] $($device.DeviceName): $failureReason" -ForegroundColor Red
        }

        $selections     = if ($result) { $result.Selections     } else { $null }
        if ($result -and $result.ErrorReason) { $failureReason = $result.ErrorReason }
        ...
        $rcgHost          = if ($result) { $result.RcgHost } else { '' }
        $installationId   = if ($result) { $result.InstallationId } else { '' }
        $installationMode = if ($result) { $result.InstallationMode } else { '' }
        ...
        $done = $xb.Count
        $pct = [int]($done * 100 / [Math]::Max(1, $total))
        ...
        [PSCustomObject]@{ ProgressEvent = $true; DeviceName = $device.DeviceName; Success = ($null -ne $selections) }
    } -ThrottleLimit $DeviceThrottle | ForEach-Object {
        if ($_.ProgressEvent) {
            $completedFirstPass++
            $pct = [int]($completedFirstPass * 100 / [Math]::Max(1, $totalDevices))
            $remaining = $totalDevices - $completedFirstPass
            Write-Progress -Id 1 -Activity "RCG Selections — $Script:PartnerName" `
                -Status "[$completedFirstPass/$totalDevices] $($_.DeviceName) — done ($remaining left)" `
                -PercentComplete $pct
        }
    }
    ...
    foreach ($item in $resultBag) {
        ...
        if ($null -eq $item.Selections -and (Test-RcgErrorReason -Reason $item.FailureReason)) {
            $rcgErrorMap[[string]$item.Device.AccountID] = [PSCustomObject]@{
                AccountID = $item.Device.AccountID; DeviceName = $item.Device.DeviceName
                PartnerName = $item.Device.PartnerName; OS = $item.Device.OS
                ProfileID = $item.Device.ProfileID; Profile = $item.Device.Profile
                StorageStatus = $item.Device.StorageStatus
                RcgHost = $item.RcgHost; InstallationId = $item.InstallationId; InstallationMode = $item.InstallationMode
                TimeStamp = $item.Device.TimeStamp; LastSuccess = $item.Device.LastSuccess
                Reason = if ($item.FailureReason) { $item.FailureReason } else { 'No selections returned' }
                LoggedAt = (Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
            }
        } elseif ($null -ne $item.Selections) {
            $rcgErrorMap.Remove([string]$item.Device.AccountID)
        }
    ...
    if ($retryBag.Count -gt 0) {
        ...
        $completedImmediate = 0
        $retryThrottle = [Math]::Max(1, [int][Math]::Ceiling($DeviceThrottle * 2 / 3))
        Write-Host "  Immediate retry throttle: $retryThrottle (two-thirds of DeviceThrottle $DeviceThrottle)" -ForegroundColor DarkGray
        Write-Progress -Id 1 -Activity "RCG Selections — IMMEDIATE RETRY — $Script:PartnerName" `
            -Status "[0/$totalImmediate] starting..." -PercentComplete 0
        ...
        $retryBag | ForEach-Object -Parallel {
            ...
            if ($using:RetryAuthDelayMs -gt 0) {
                $jitter = Get-Random -Minimum 0 -Maximum ($using:RetryAuthDelayMs + 1)
            ...
            try {
                ...
                if ($result -and $result.Selections) {
                    ...
                    $failureReason = if ($result -and $result.ErrorReason) { $result.ErrorReason } else { 'No selections returned' }
                    Write-Host "  [-] $($device.DeviceName): retry - no selections returned" -ForegroundColor Yellow
                }
            } catch {
                $failureReason = $_.Exception.Message
                Write-Host "  [-] $($device.DeviceName): retry - $failureReason" -ForegroundColor Red
            }
            ...
            $done = $ib.Count
            $total = $using:totalImmediate
            $pct = [int]($done * 100 / [Math]::Max(1, $total))
            $remaining = $total - $done

            if ($success) {
                Write-Host "  [+] $($device.DeviceName): retry OK   $pct% ($done/$total, $remaining left)" -ForegroundColor Green
            } else {
                Write-Host "  [-] $($device.DeviceName): retry failed   $pct% ($done/$total, $remaining left)" -ForegroundColor Yellow
            }
            [PSCustomObject]@{ ProgressEvent = $true; DeviceName = $device.DeviceName; Success = $success }
        } -ThrottleLimit $retryThrottle | ForEach-Object {
            if ($_.ProgressEvent) {
                $completedImmediate++
                $pct = [int]($completedImmediate * 100 / [Math]::Max(1, $totalImmediate))
                $remaining = $totalImmediate - $completedImmediate
                Write-Progress -Id 1 -Activity "RCG Selections — IMMEDIATE RETRY — $Script:PartnerName" `
                    -Status "[$completedImmediate/$totalImmediate] $($_.DeviceName) — done ($remaining left)" `
                    -PercentComplete $pct
            }
        }

        Write-Progress -Id 1 -Activity "RCG Selections — IMMEDIATE RETRY" -Completed
        ...
        foreach ($item in $immediateBag) {
            if ($item.Success) {
                ...
                if (Test-RcgErrorReason -Reason $item.FailureReason) {
                    $rcgErrorMap[[string]$item.Device.AccountID] = [PSCustomObject]@{
                    AccountID = $item.Device.AccountID; DeviceName = $item.Device.DeviceName
                    PartnerName = $item.Device.PartnerName; OS = $item.Device.OS
                    ProfileID = $item.Device.ProfileID; Profile = $item.Device.Profile
                    ...
                    RcgHost = $item.RcgHost; InstallationId = $item.InstallationId; InstallationMode = $item.InstallationMode
                    TimeStamp = $item.Device.TimeStamp; LastSuccess = $item.Device.LastSuccess
                    Reason = if ($item.FailureReason) { $item.FailureReason } else { 'Retry returned no selections' }
                    LoggedAt = (Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
                    }
                } else {
                    $rcgErrorMap.Remove([string]$item.Device.AccountID)
    ...
    ## ======== RETRY PASS ========
    ## The automatic immediate retry above is the only device-level retry.
    ## RetryFailed is retained as a compatibility parameter but no longer starts a third pass.
    if ($false -and -not $RetryOnly -and $RetryFailed -and $retryBag.Count -gt 0) {
        Write-Output $Script:strLineSeparator
        ...
        foreach ($item in $retryResultBag) {
            ...
            if ($null -eq $item.Selections -and (Test-RcgErrorReason -Reason $item.FailureReason)) {
                $rcgErrorMap[[string]$item.Device.AccountID] = [PSCustomObject]@{
                    ...
                    RcgHost = $item.RcgHost; InstallationId = $item.InstallationId; InstallationMode = $item.InstallationMode
                    TimeStamp = $item.Device.TimeStamp; LastSuccess = $item.Device.LastSuccess
                    Reason = if ($item.FailureReason) { $item.FailureReason } else { 'Final retry returned no selections' }
                    LoggedAt = (Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
                }
            } elseif ($null -eq $item.Selections) {
                $rcgErrorMap.Remove([string]$item.Device.AccountID)
            } elseif ($null -ne $item.Selections) {
                $rcgErrorMap.Remove([string]$item.Device.AccountID)
            }
    ...
    } elseif ($false -and -not $RetryOnly -and $retryBag.Count -gt 0) {
        ## Legacy third-pass output path intentionally disabled.
    ...
    ## ---- Write final unrecovered RCG error log ----
    ## Devices recovered during either retry pass are removed from $rcgErrorMap.
    if ($rcgErrorMap.Count -gt 0) {
        $rcgErrorPath = "$OutputPath\$($CurrentDate)_RCG_Errors_$($Script:PartnerName -replace '[^a-zA-Z_0-9]','').csv"
        @($rcgErrorMap.Values | Sort-Object DeviceName) |
            Export-Csv -Path $rcgErrorPath -NoTypeInformation -Encoding UTF8 -Force
        Write-Host "  Final unrecovered RCG errors: $rcgErrorPath ($($rcgErrorMap.Count) device(s))" -ForegroundColor Yellow
    } else {
        Write-Host "  No unrecovered RCG errors after retry processing." -ForegroundColor Green
    }
...
#region ----- Report ----
    ...
    Write-Host "  Generating final analysis report and export files..." -ForegroundColor Cyan
    Write-Progress -Id 2 -Activity "Final report" -Status "Building analysis and exports..." -PercentComplete 10
    ...
    if ($Export -and $masterHashForUse.Count -gt 0) {
        Write-Progress -Id 2 -Activity "Final report" -Status "Writing CSV and Excel exports..." -PercentComplete 90
    ...
    Write-Progress -Id 2 -Activity "Final report" -Completed
