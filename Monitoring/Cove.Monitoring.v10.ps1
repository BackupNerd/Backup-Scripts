...
[CmdletBinding()]
    Param (
        [Parameter(Mandatory=$False)][ValidateSet("FileSystem","SystemState","VMware","VssHyperV","VssMsSql","Exchange","MySql","NetworkShares","VssSharePoint","Oracle")] $Datasource,
        [Parameter(Mandatory=$False)] [int]$SuccessHoursInVal = 24,     ## Throw warning if > than this number of hours since last success
...
Function Get-BackupState {
    $Script:BackupService = get-service -name "Backup Service Controller" -ea Stop
...
Function Get-BackupSelections {
    ...
    if ($Null -eq ($FunctionalProcess)) { Write-Warning "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $BackupSelections = & $clienttool -machine-readable control.selection.list -delimiter "'" | ConvertFrom-String -Delimiter "'" | Select-Object -skip 1 -property @{N='DataSource'; E={$_.P1}},@{N='Type'; E={$_.P2.replace("Inclusive","Inc (+)").replace("Exclusive","Exc (-)")}},@{N='Path'; E={$_.P4}} | where-object {$_.DataSource -eq $datasource}}catch{ Write-Warning "ERROR     | $_" }}
