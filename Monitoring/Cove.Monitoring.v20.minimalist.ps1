<# ----- About: ----
    # N-able | Cove Data Protection | Monitor (Minimal)
    # Revision v20 - 2025-02-11
...
<# ----- Behavior: ----
    ...
    #
    #  Suggested use withP owershell Script Monitors (Connectwise or other)
...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)] [int]$WrkStnSuccessHoursInVal = 72,   ## Throw warning if > than this number of hours since last success - Workstation
        [Parameter(Mandatory=$False)] [int]$ErrorLimit = 10,                ## Maximum number of error messages to display per data source
        [Parameter(Mandatory=$False)] [int]$SynchThreshold = 90             ## Throw warning if LSV Sync is < than this percentage
...
Function Get-BackupState {
    ...
    Write-Verbose "`n[Backup Service Controller]"
    if ($BackupService.status -eq "Running"){
        $Script:BackupServiceOutVal = 1
        $Script:BackupServiceOutTxt = $($BackupService.status)
        Write-Verbose  "Service Status            | $BackupServiceOutTxt"
...
Function Get-FixedVolumes {
    ...
    Write-Verbose "`n[Detecting $datasource DataSources]"
    Write-Verbose "Fixed Volumes             | $FixedVolumeString"
...
Function Get-USBVolumes {
    ...
    # ----- End Get Disk Partion Letter for USB / Non USB Bustypes ----
        if ($USBvol) {
            ...
            Write-Verbose "USB Volumes               | $($USBvolString.replace("\\","\")) "
        }
    }
    ...
    if ($Null -eq ($FunctionalProcess)) { 
        ...
        try { # Command(s) to try
            ...
            if ($Filters) {
                ...
                Write-Verbose "`n[$Datasource Filters]"
                Write-Verbose "Runtime Exclusion Filters | $($FilterString.replace("\\","\"))"
            }
        }
        catch{ # What to do with terminating errors
        }
                    ...
                    if ($StatusValue -contains $BackupStatus) { 
                        ...
                        try { # Command(s) to try
                            ...
                            If ($null -ne $Script:sessionerrors) {
                                ...
                                $global:failed = 1
                                $Script:sessionerrors | Select-Object Datetime,Content,Path | Sort-Object -Descending Datetime | Format-Table
...
Function Get-StatusReportBase {
    ...
    if (Test-Path $script:StatusReportxml ) {    
        ...
        Write-Verbose "Profile Id                | $Script:ProfileIdOutVal"
        Write-Verbose "Profile Ver               | $Script:ProfileVersionOutVal"
    ...
    if (Test-Path $script:StatusReportxml ) {    
        ...
        if ($PluginColorBar) {
            ...
            if ($null -eq $PluginLastSuccessfulSessionStatus ) {$Script:LastSuccessStatusOutTxt = "Never"} else {
                ...
                Write-Verbose "Retention                 | $Script:PluginRetentionOutVal $Script:RetentionUnitsOutTxt "   
                
                Write-Verbose "Last Success (UTC)        | $Script:LastSuccessTimeUTCOutTxt $Script:LastSuccessStatusOutTxt"
                Write-Verbose "Last Complete (UTC)       | $Script:LastCompleteTimeUTCOutTxt $Script:LastCompleteStatusOutTxt"      
                Write-Verbose "Last Session (UTC)        | $Script:LastSessionTimeUTCOutTxt $Script:LastSessionStatusOutTxt"
                Write-Verbose "Session Duration (HRS)    | $Script:PluginSessionDurationHrsOutVal"
                Write-Verbose "Success Check (HRS)       | $SuccessHoursInVal"
            ...
            if ($Script:LastErrorCountOutVal -ge 1) {Get-BackupErrors $DataSource $ErrorLimit}
            Start-Sleep -Seconds 3

        }else{Write-output "No Prior $Datasource Backup has completed"; $global:failed = 1} ## Prevent null values
...
Function Get-Status ([int]$SynchThreshold) {
    ...
    Write-Verbose "`n[LocalSpeedVault Status]"
    Write-Verbose "LSV Enabled               | $Script:LSVEnabledOutTxt"
    Write-Verbose "LSV Sync Status           | $Script:LSVSyncStatusOutTxt"
    Write-Verbose "Cloud Sync Status         | $Script:CloudSyncStatusOutTxt"
    Write-Verbose "Selected Size (GB)        | $Script:TotalSelectedGBOutTxt"        
    Write-Verbose "Used Storage (GB)         | $Script:TotalUsedGBOutTxt"
    Write-Verbose "Last LSV Path             | $LSVPath"
    Write-Verbose "Last LSV User             | $LSVUser"
...
foreach ($datasource in $datasources) {
    ...
    Switch ($script:datasource) { ## "FileSystem","SystemState","VMware","VssHyperV","VssMsSql","Exchange","MySql","NetworkShares","VssSharePoint"
        ...
        'MySql' {
            ...
            if ($DatasourcePresent -eq $null) {Write-Output "$Datasource selections configured but no $Datasource installation detected"; <#$global:failed = 1#>}
            #else {$DatasourcePresent[-1] | Format-Table}
            #Get-BackupSelections
            }
        'Oracle' {
...
Write-Output "`nExit Code: $LASTEXITCODE"