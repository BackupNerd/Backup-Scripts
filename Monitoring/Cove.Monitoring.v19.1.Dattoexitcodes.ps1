...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)][ValidateSet("Nsight","Datto")] [string]$RmmPlatform = "Datto",  ## Selects monitor output / exit-code convention
        [Parameter(Mandatory=$False)] [int]$NsightFailExitCode = 1001,      ## Exit code on failure when -RmmPlatform Nsight
        [Parameter(Mandatory=$False)] [int]$DattoFailExitCode = 1           ## Exit code on failure when -RmmPlatform Datto
    )
Clear-Host      
...
Function Set-Failed ($Message) {
    ...
    $Script:FailureReasons += $Message
    $global:failed = 1
} ## Record a failure reason (for Datto Result block) and flag the run as failed
...
Function Get-BackupState {
    ...
    if ($BackupService.status -eq "Running"){
        ...
        $Script:BackupServiceOutTxt = $($BackupService.status)
        Set-Failed "Service Status   | $BackupServiceOutTxt"
        break
...
Function Get-Datasources {
    ...
    if ($Null -eq ($FunctionalProcess)) {
        Set-Failed "Backup Manager Not Running"
        }else{
            Do {
                    ...
                    if ($StatusValue -contains $BackupStatus) { 
                        ...
                        try { # Command(s) to try
                            ...
                            If ($null -ne $Script:sessionerrors) {
                                ...
                                Set-Failed "[$datasource] Errors Found"
...
Function Get-StatusReportBase {
    ...
    else{Set-Failed "[$script:StatusReportxml] not found"}
...
Function Get-StatusReport {
    ...
    if (Test-Path $script:StatusReportxml ) {    
            ...
            if ($null -eq $PluginLastSuccessfulSessionStatus ) {$Script:LastSuccessStatusOutTxt = "Never"} else {
                ...
                if ($LastSuccessTimeUTCOutTxt -gt 0) { 
                    ...
                    if ($HoursSinceLastOutVal -le $SuccessHoursInVal) {
                        ...
                        Set-Failed "[$Datasource] Last Success | $HoursSinceLastOutVal(HRS) Ago"
                    }
                }
            }
        ...
        }else{Set-Failed "[$Datasource] No Prior Backup has completed"} ## Prevent null values
...
Function Get-Status ([int]$SynchThreshold) {
    ...
    if (($LocalSpeedVaultEnabled -eq 1) -and (($BackupServerSynchronizationStatus -eq "Failed") -or ($LocalSpeedVaultSynchronizationStatus -eq "Failed"))) {Set-Failed "LSV Failed"}

    elseif (($Script:LocalSpeedVaultEnabled -eq 1) -and ($BackupServerSynchronizationStatus -ne "synchronized")) {
        if ( ($BackupServerSynchronizationStatus.replace("%","")/1 -lt $SynchThreshold )){Set-Failed "Cloud sync is below $SynchThreshold%"}
    ...
    elseif (($LocalSpeedVaultEnabled -eq 1) -and ($LocalSpeedVaultSynchronizationStatus -ne "synchronized")) {
        if ( ($LocalSpeedVaultSynchronizationStatus.replace("%",""))/1 -lt $SynchThreshold ){Set-Failed "LSV sync is below $SynchThreshold%"}
    }  ## Warn if Sync % is below threshold
...
foreach ($datasource in $datasources) {
    ...
    Switch ($script:datasource) { ## "FileSystem","SystemState","VMware","VssHyperV","VssMsSql","Exchange","MySql","NetworkShares","VssSharePoint"
        ...
        'MySql' {
            ...
            else{Set-Failed "[$Datasource] No installation detected"}
            Get-BackupSelections
            Get-StatusReport
            }
        'Oracle' {
...
#If $global:failed is 1, cause the RMM scriptcheck/monitor to fail in dashboard
if ($RmmPlatform -eq "Datto") {
    ## Datto RMM component monitors parse the alert message from between these tags
    ## Output must begin with "STATUS=" (no space after the "=") or Datto shows "no data"
    Write-Output "`n<-Start Result->"
    if ($global:failed -eq 1) {
        Write-Output "STATUS=$($Script:FailureReasons -join " | ")"
    } else {
        Write-Output "STATUS=OK - Cove Data Protection checks passed"
    }
    Write-Output "<-End Result->"

    if ($global:failed -eq 1) { Exit $DattoFailExitCode } else { Exit 0 }
} else {
    ## N-sight RMM scriptcheck: triggers on the 'Warning:' keyword in output; exit code is informational
    if ($global:failed -eq 1) { Exit $NsightFailExitCode } else { Exit 0 }
}