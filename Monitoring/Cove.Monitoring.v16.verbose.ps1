...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)] [int]$ServiceRestartAttempts = 2,     ## Maximum number of error messages to display per data source
...
Function Get-BackupState {
    ...
    elseif (($BackupService.status -ne "Running") -and ($null -ne $BackupService.status )) {
        ...
        if(($BackupServiceOutTxt -eq "Stopped") -and ($ServiceRestartAttempts -le $Script:RestartAttempt)) {
            Write-Warning "Backup Service Disabled or Failed to Restart after $Script:RestartAttempt attempts"
            $global:failed = 1
            Break
        }
        else {
            $Script:RestartAttempt ++
            Write-Output "Start Services attempt $Script:RestartAttempt"
            Start-service -Name 'Backup Service Controller'
            #debug Stop-Service -Name 'Backup Service Controller' -Force
                    ...
                    if ($StatusValue -contains $BackupStatus) { 
                        ...
                        try { # Command(s) to try
                            ...
                            If ($null -ne $Script:sessionerrors) {
                                ...
                                Write-Warning "[$datasource] $Script:LastErrorCountOutVal Errors Found"
                                $global:failed = 1
...
foreach ($datasource in $datasources) {
    ...
    Switch ($script:datasource) { ## "FileSystem","SystemState","VMware","VssHyperV","VssMsSql","Exchange","MySql","NetworkShares","VssSharePoint"
        ...
        'MySql' {
            ...
            if ($DatasourcePresent -eq $null) {Write-Warning "$Datasource configured but no $Datasource installation detected"; $global:failed = 1}
