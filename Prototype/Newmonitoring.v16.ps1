...
Function Get-SystemInfo {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; [array]$BackupSystemInfo = & $clienttool system-info.get | convertfrom-json }catch{ Write-Warning "ERROR     : $_" }}
...
Function Get-InitError {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $initerror = & $clienttool control.initialization-error.get  | convertfrom-json}catch{ Write-Warning "ERROR     : $_" }}
...
Function Get-BackupFPVisa {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $BackupFPVisa = & $clienttool in-agent-authentication-token.get -config-path "$config" | convertfrom-json }catch{ Write-Warning "ERROR     : $_" }} 
    Write-output "`n[Backup Manager]"
...
Function Get-VssStatus {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $VssStatus = & $clienttool vss.check }catch{ Write-Warning "ERROR     : $_" }}
...
Function Get-StorageStatus {
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop' ; $StorageStatus = & $clienttool storage.test }catch{ Write-Warning "ERROR     : $_" }}
...
Function Get-BackupSettings {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $BackupSettings = & $clienttool control.setting.list }catch{ Write-Warning "ERROR     : $_" }}
...
Function Get-BackupSelections {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $BackupSelections = & $clienttool control.selection.list }catch{ Write-Warning "ERROR     : $_" }}
    ...
    foreach ($datasource in  $Datasources ) {
        if ($Null -eq ($FunctionalProcess)) {
             "Backup Manager Not Running" 
                ...
                try { # Command(s) to try
                    $ErrorActionPreference = 'Stop'
                    $sessionerrors.$datasource = & $clienttool control.session.error.list -datasource $datasource
...
Function Get-BackupFilters {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $BackupFilters = & $clienttool control.filter.list }catch{ Write-Warning "ERROR     : $_" }}
...
Function Get-BackupSchedules {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $BackupSchedules = & $clienttool control.schedule.list }catch{ Write-Warning "ERROR     : $_" }}
...
Function Get-ArchiveSchedules {
    ...
    if ($Null -eq ($FunctionalProcess)) { "Backup Manager Not Running" }else{ try { $ErrorActionPreference = 'Stop'; $ArchiveSchedules = & $clienttool control.archiving.list }catch{ Write-Warning "ERROR     : $_" }}
...
Function Get-StatusReport{
    $Script:StatusReportxml = "C:\ProgramData\MXB\Backup Manager\StatusReport.xml" ## StatusReport.XML to Parse

    [xml]$StatusReport = get-content -path $Script:StatusReportxml 
...
Function Get-StatusReport2{
    ...
    if ($PluginFileSystemColorBar) {
        ...
        "Last Session (UTC)      : $PluginFileSystemLastSessionTimestamp $PluginFileSystemLastSessionStatus "
        "Last Success (UTC)      : $PluginFileSystemLastSuccessfulSessionTimestamp $PluginFileSystemLastSuccessfulSessionStatus"
        "Last Complete (UTC)     : $PluginFileSystemLastCompletedSessionTimestamp $PluginFileSystemLastCompletedSessionStatus"
        ...
        if($statuscode -notlike "Backup, Files and folders*") {
            ...
            "Last Sent/Sel Size%     : $(($PluginFileSystemLastSessionSentSize/$PluginFileSystemLastSessionSelectedSize)*100)"
                ...
                if ($PluginFileSystemPreRecentSessionSelectedSize) {
                "`n[FileSystem Size Change]"
    ...
    if ($PluginVssSystemStateColorBar) {
        ...
        "Last Session (UTC)      : $PluginVssSystemStateLastSessionTimestamp $PluginVssSystemStateLastSessionStatus "
        "Last Success (UTC)      : $PluginVssSystemStateLastSuccessfulSessionTimestamp $PluginVssSystemStateLastSuccessfulSessionStatus"
        "Last Complete (UTC)     : $PluginVssSystemStateLastCompletedSessionTimestamp $PluginVssSystemStateLastCompletedSessionStatus"
        ...
        if($statuscode -notlike "Backup, System state (VSS)*") {
            ...
            if ($PluginVssSystemStatePreRecentSessionSelectedSize) {
                "`n[SystemState Size Change]"
...
Function Get-LSVstatus ($SynchThreshold = 80) {
    ...
    $LSVPath                                    = $(Get-BackupSettings | Where-Object { $_ -like "LocalSpeedVaultLocation *"}).replace("LocalSpeedVaultLocation ","")
    $LSVUser                                    = $(Get-BackupSettings | Where-Object { $_ -like "LocalSpeedVaultUser *"}).replace("LocalSpeedVaultUser     ","")
    }
    ...
    if ($DocumentsEnabled -eq 1) { 
        ...
        "`n[LocalSpeedVault Status]"
        "Device                  : $Device"
        "Machine                 : $MachineName"
        "TimeStamp (UTC)         : $(Convert-UnixTimeToDateTime $timestamp)"
        "TimeStamp (UTC)         : $(Convert-UnixTimeToDateTime $timestamp)"
        "LSV Enabled             : $($LocalSpeedVaultEnabled.replace("1","True").replace("0","False"))"
        "LSV Sync Status         : $LocalSpeedVaultSynchronizationStatus"
        "Cloud Sync Status       : $BackupServerSynchronizationStatus"
        "Est Space Required      : $($UsedStorage | RND GB 5)"
        "Last LSV Path           : $LSVpath"
        "Last LSV User           : $LSVUser"
        ...
        if (($LocalSpeedVaultEnabled -eq 1) -and ($BackupServerSynchronizationStatus -ne "synchronized")) { 
            if ( ($BackupServerSynchronizationStatus.replace("%","")/1 -le $SynchThreshold )){Write-Warning "Cloud sync is below $SynchThreshold%"}
        } ## Warn if Sync % is below threshold
        
    
        if (($LocalSpeedVaultEnabled -eq 1) -and ($LocalSpeedVaultSynchronizationStatus -ne "synchronized")) { 
            if ( ($LocalSpeedVaultSynchronizationStatus.replace("%",""))/1 -le $SynchThreshold ){Write-Warning "LSV sync is below $SynchThreshold%"}
        }  ## Warn if Sync % is below threshold
...
Function Test-Documents {
    ...
    if (($null -eq $FunctionalProcess) -and ((Test-Path $Configinipath -PathType leaf) -eq $false)) {
        ...
        $DocumentsEnabled = ([Xml] (get-content $StatusReportxml)).SelectSingleNode("//AntiCryptoEnabled")."#text"
        Write-Output "Documents = $($DocumentsEnabled.replace("0","False").replace("1","True"))"
    }
}  ## Basic Test if Documents is installed
...
Function Get-BackupProductOsType {
    ...
    if (($null -eq $FunctionalProcess) -and ((Test-Path $Configinipath -PathType leaf) -eq $false)) {
        ...
        if ((Test-Path $StatusReportxml -PathType leaf) -eq $false) {
            ...
            if (($ProductType) -and ($OsType)) { $ProductOsType = "$ProductType $OsType" }
...
Function Get-OsType {
    ...
    if ($MyOS.Caption -like "*Server*") {
        Write-Output "$($MyOS.Caption)"
        $OsType = "Server"
        $OSType
    }
    elseif ($MyOS.Caption -notlike "*Server*") {
        Write-Output "$($MyOS.Caption)"
        $OsType = "Workstation"
        $OSType
    }
    $OsStatusCode = $OsStatus["$OSType"]
    $OsStatusCode
}  ## Get OS Type
...
Get-InitError;Get-ApplicationStatus;Get-JobStatus;Get-VssStatus;Get-LSVstatus       
