...
<# ----- Behavior: ----
    ...
    #   Cove Data Protection retains the LAST backup session of each day for up to 30 days.
    #   When intra-day sessions expire, only the files present in the FINAL session of that
    #   day survive in the 30-day daily retention tier.
    #
    ...
    #   -SmartClear  Automatically clear selections on the first run of each new calendar
    #                day, then accumulate on all subsequent runs that day.
    #                Recommended for scheduled tasks — retains full daily coverage.
    ...
    #   .\.CoveBackupNow.v01.ps1 -ScanPath "C:\Data" -SmartClear          # recommended for scheduled tasks
...
Param (
    ...
    [Parameter(Mandatory=$false,  HelpMessage="Clear selections only on the first run of each new calendar day; accumulate on all subsequent runs that day (recommended for scheduled tasks)")]
    [bool]$SmartClear=$true,
...
#region ----- Environment, Variables, Names and Paths ----
    ...
    if ($ClearFirst -and $SmartClear) {
        Write-Error "-ClearFirst and -SmartClear cannot be used together. Use -SmartClear for scheduled tasks or -ClearFirst for a one-off clean slate."
        exit 1
    }
...
#region ----- Functions ----
    ...
    function Invoke-ClientToolFileSelections {
        ...
        Write-Host "  Applying | $($Paths.Count) selection(s) in $totalBatches batch(es) of up to $BatchSize..." -ForegroundColor Cyan
        ...
        for ($i = 0; $i -lt $Paths.Count; $i += $BatchSize) {
            ...
            Set-ClientToolSelection -DataSource FileSystem -Include $batch -Priority $Priority
        }
...
#region ----- Main Script ----
    ...
    if ($PSCmdlet.ShouldProcess($SelectionFile, "Write selection list")) {
        Write-SelectionListFile -Paths $filePaths -OutputPath $SelectionFile
    }
    ...
    if ($ClearFirst) {
        ...
        if ($PSCmdlet.ShouldProcess("FileSystem", "Clear existing selections")) {
            ...
            (Get-Date).ToString('yyyy-MM-dd') | Out-File -FilePath $SmartClearStateFile -Encoding UTF8 -NoNewline
        }
    } elseif ($SmartClear) {
        # Clear only on the first run of a new calendar day.
        # The last-clear date is persisted in CoveBackupNow_LastClear.dat so that
        # repeated runs throughout the day accumulate selections rather than wiping them.
        $today         = (Get-Date).ToString('yyyy-MM-dd')
        $lastClearDate = if (Test-Path $SmartClearStateFile) { (Get-Content $SmartClearStateFile -Raw).Trim() } else { '' }

        if ($lastClearDate -ne $today) {
            Write-Host "  SmartClear | First run of a new day (last clear: $(if ($lastClearDate) { $lastClearDate } else { 'never' })) — clearing selections..." -ForegroundColor Cyan
            if ($PSCmdlet.ShouldProcess("FileSystem", "Clear existing selections (SmartClear — new day)")) {
                Clear-ClientToolSelection -DataSource FileSystem -Confirm:$false
                $today | Out-File -FilePath $SmartClearStateFile -Encoding UTF8 -NoNewline
            }
        } else {
            Write-Host "  SmartClear | Already cleared today ($today) — accumulating selections" -ForegroundColor DarkCyan
        }
    } else {
        Write-Host "  Accumulating | Adding to existing FileSystem selections (use -SmartClear or -ClearFirst to reset)" -ForegroundColor DarkCyan
    }

    Invoke-ClientToolFileSelections -Paths $filePaths
