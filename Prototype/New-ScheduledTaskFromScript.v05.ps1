...
Function Get-PowerShellExecutable {
    ...
    # PowerShell 7 locations - check known install paths first
    $ps7Paths = [System.Collections.Generic.List[string]]@(
        "${env:ProgramFiles}\PowerShell\7\pwsh.exe",
        "${env:ProgramFiles(x86)}\PowerShell\7\pwsh.exe",
        "${env:ProgramFiles}\PowerShell\7-preview\pwsh.exe"
    )

    # Add Windows Store / MSIX installs (WindowsApps) - dynamically discovered
    $windowsAppsBase = Join-Path $env:ProgramFiles "WindowsApps"
    if (Test-Path $windowsAppsBase) {
        Get-ChildItem -Path $windowsAppsBase -Filter "Microsoft.PowerShell_*" -Directory -ErrorAction SilentlyContinue |
            Sort-Object Name -Descending |
            ForEach-Object {
                $candidate = Join-Path $_.FullName "pwsh.exe"
                if (Test-Path $candidate) { $ps7Paths.Add($candidate) }
            }
    }

    # Fallback: resolve via PATH / Get-Command
    $ps7FromPath = Get-Command pwsh -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Source
    if ($ps7FromPath -and (Test-Path $ps7FromPath)) {
        $ps7Paths.Add($ps7FromPath)
    }
