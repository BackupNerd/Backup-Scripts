<# ----- About: ----
    # Cove Bulk Generate Redeploy Creds - Minimal
    # Revision v35 - 2023-03-09 
...
<# ----- Behavior: ----
    ...
    # Export to CSV
    # 
    # Script to bulk generates Backup Manager reinstallation credentials and commands in RESTORE ONLY MODE
    # for your backup devices that use passphase based encryption key management.
    # Generates CSV files with download and installer scripts with individual device credentials
    #
    # Provides a CMD line compatible script for Powershell 2.0 and later environments with HTTPS & HTTP download options for legacy operating systems
    #
    # Use the -DeviceCount ## (default=5000) parameter to define how many devices to process
    ...
    #
    # https://documentation.n-able.com/covedataprotection/USERGUIDE/documentation/Content/backup-manager/backup-manager-installation/get-passphrase.htm
    # https://documentation.n-able.com/covedataprotection/USERGUIDE/documentation/Content/backup-manager/backup-manager-installation/re-install.htm
...
[CmdletBinding()]
    Param (
        [Parameter(Mandatory=$False)] [int]$DeviceCount = 5000,                    ## Change default Number of devices to lookup
...
#region ----- Environment, Variables, Names and Paths ----
    ...
    $csvcommands = "$ExportPath\$($CurrentDate)_CONFIDENTIAL_BackupManagerCredentials.csv"
    #$WinDownload = '[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (New-Object System.Net.WebClient).DownloadFile("https://cdn.cloudbackup.management/maxdownloads/mxb-windows-x86_x64.exe","C:\Users\Public\Downloads\mxb-windows-x86_x64.exe")'
    #$WincmdDownload = "powershell -command ([Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12); ((New-Object System.Net.WebClient).DownloadFile('https://cdn.cloudbackup.management/maxdownloads/mxb-windows-x86_x64.exe','C:\Users\Public\Downloads\mxb-windows-x86_x64.exe'))"
    #$WinLegacyDownload = "powershell -command ((New-Object System.Net.WebClient).DownloadFile('http://cdn.cloudbackup.management/maxdownloads/mxb-windows-x86_x64.exe','C:\Users\Public\Downloads\mxb-windows-x86_x64.exe'))"
    ...
    # (Create the JSON object to call the EnumerateAccountStatistics function)
        ...
        Add-Member -PassThru NoteProperty params @{
                                                    query = @{
                                                                      ...
                                                                      Filter="$BackupFilter"
                                                                      Columns=("AU","AR","AN","MN","OP","OT","OS","PF","PN","QW","CD","TS","TL","I82")
                                                                      StartRecordNumber = 0
    ...
    # (No error)
        ...
        ForEach ( $Result in $EnumerateAccountStatisticsSession.result.result ) {
            $Script:DeviceDetail += New-Object -TypeName PSObject -Property @{ AccountID    = [String]$Result.AccountId;
                                                                        ...
                                                                        IsPassphrase = $Result.Settings.I82 -join '' ;
        ...
        if ($silent) {
            $Script:Detail = $DeviceDetail | sort-object ComputerName
        }else{
            $Script:Detail = $DeviceDetail | sort-object ComputerName | out-gridview -OutputMode Multiple -Title "Select all Required Devices and Click OK to Request PassPhrases"
        }
...
#Write-Output "partnername`tostype`tosversion`tcreation`tlastsuccess`ttimestamp`tid`tcomputername`tdevicename`tIsPassphrase`tpassword`tpassphrase`tSecureTLSDownloadandInstallCommand`tLegacySecurityDownloadandInstallCommand" | Out-file $csvcommands

#Write-output "computername___`t`t`t`tdevicename______`t`t`t`tinstallkey__`tpassphrase__________________________`tcreation_______`ttimestamp_______`tlastsuccess_______" | Out-file $csvcommands

Write-output "computername___`t`tdevicename______`t`tinstallkey__`t`tpassphrase__________________________`tcreation___________`ttimestamp__________`tlastsuccess_______" | Out-file $csvcommands
    ...
    Foreach ($d in $Detail) {
        ...
        #$ComputerName.length
        if ($ComputerName.length -lt 10) { $ComputerName = $ComputerName + '        '}
        #$ComputerName.length

        $DeviceName = $d.DeviceName
        #$devicename.length
        if ($DeviceName.length -lt 15) { $DeviceName =  $DeviceName + '        '}
...
# (Call the JSON Web Request Function to get the GetPassphrase Object)
        ...
        If (($d.OSVersion -match "Windows") -and ($passphrase -ne "")) {
        #Write-output ("$computername`t`t`t`t$devicename`t`t`t`t$password`t$passphrase`t$creation`t$timestamp`t$lastsuccess") | Out-file $csvcommands -append

        Write-output ("$computername`t`t$devicename`t`t$password`t`t$passphrase`t$creation`t$timestamp`t$lastsuccess") | Out-file $csvcommands -append
        }
    ...
    Write-output "  Credential file found here:"
    Write-output $Script:strLineSeparator
    Write-Output "  CSV Path = &Notepad `"$csvcommands`""
