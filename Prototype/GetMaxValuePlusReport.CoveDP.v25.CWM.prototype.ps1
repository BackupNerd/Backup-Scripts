...
[CmdletBinding(DefaultParameterSetName="Data Protection Plan Usage")]
    Param (
        ...
        [Parameter(ParameterSetName="Data Protection Plan Usage",Mandatory=$False)]
            [Parameter(ParameterSetName="MaxValue Usage",Mandatory=$False)][ValidateRange(0,24)] $Last = 1, ## Count back # Last Months i.e. 0 current, 1 Prior Month
        ...
        [Parameter(Mandatory=$False)][String]$CWMAgreementName = "Backup",                             ## ConnectWiseMaanger Agreement Name
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function get-reference ($Partnerid) { 
        ...
        $webrequest = Invoke-WebRequest -Method POST `
            ...
            write-output $reference.result.result.externalcode
    }
    ...
    Function Send-GetDevices {
        ...
        ForEach ( $DeviceResult in $DeviceResponse.result.result ) {
            Write-Output $DeviceResult.AccountId
    ...
    Function Send-GetDeviceHistory ($DeviceID, $DeleteDate) {
        ...
        $Script:DeviceHistoryDetail = @()
            Write-Output "Getting historic data for deleted devices"
        ForEach ( $DeviceHistory in $DeviceResponse.result.result ) {
            write-output $DeviceHistory.AccountId
...
Function Send-Webhook {
    $hookUrl = 'https://webhook.site/2f868d7f-02d0-4f62-ae8c-289989b86aee'
...
<#
        ...
        #>
            ...
            $Script:MVPlus | Where-object {$_.CustomerState -eq "InProduction"} | where-object {$_.CustomerName -notlike '*Recycle Bin'} | 
                                        ...
                                        CustomerState,CreationDate,ProductionDate,DeviceDeletionDate,Now_TimeStamp,SelectedSizeGB,UsedStorageGB | Export-CSV -path "$csvoutputfile" -delimiter "$Delimiter" -NoTypeInformation -Encoding UTF8
            ...
            #$Script:MVPlus | Where-object {$_.CustomerState -eq "InTrial"} | where-object {$_.CustomerName -notlike '*Recycle Bin'} | Select-object Parent3Name,Parent3Id,Parent2Name,Parent2Id,Parent1Name,Parent1Id,Now_Parent1Reference,CustomerName,CustomerId,Now_Reference,ComputerName,DeviceName,DeviceId,Now_Product,OsType,Now_Physicality,PhysicalServer,VirtualServer,Workstation,Documents,RecoveryTesting,O365Users,CustomerState,CreationDate,ProductionDate,DeviceDeletionDate,Now_TimeStamp,SelectedSizeGB,UsedStorageGB | Export-Excel -Path "$xlsoutputfile" -ConditionalFormat $ConditionalFormat -AutoFilter -AutoSize -TableName TrialUsage -FreezeTopRowFirstColumn -WorksheetName "TRIAL $(get-date $period -UFormat `"%b-%Y`")" -tablestyle Medium6 -BoldTopRow
            ...
            #$Script:MVPlus | Where-object {$_.CustomerState -eq "InProduction"} | where-object {$_.CustomerName -notlike '*Recycle Bin'} | Select-object Parent3Name,Parent3Id,Parent2Name,Parent2Id,Parent1Name,Parent1Id,Now_Parent1Reference,CustomerName,CustomerId,Now_Reference,ComputerName,DeviceName,DeviceId,Now_Product,OsType,Now_Physicality,PhysicalServer,VirtualServer,Workstation,Documents,RecoveryTesting,O365Users,CustomerState,CreationDate,ProductionDate,DeviceDeletionDate,Now_TimeStamp,SelectedSizeGB,UsedStorageGB | Export-Excel -Path "$xlsoutputfile" -ConditionalFormat $ConditionalFormat -AutoFilter -AutoSize -TableName ProductionUsage -FreezeTopRowFirstColumn -WorksheetName (get-date $period -UFormat "%b-%Y") -tablestyle Medium6 -BoldTopRow -IncludePivotTable -PivotRows Parent1Name,CustomerName -PivotDataToColumn -PivotData @{PhysicalServer='sum';VirtualServer='sum';Workstation='sum';Documents='sum';RecoveryTesting='count';O365Users='sum'}
    ...
    if ($SendtoCWM) {
        ...
        Function UpdateQty {
            ...
            $Additions | Out-GridView -Title "$($CDPusage.partnername) >>> $($CWMcompany.name) | company.id $($CWMcompany.id) | agreement.id $($Agreement.id) | Addition IDs & QTYs before updates"
            ...
            foreach ($Addition in $additions) {
                ...
                if ($Addition.product.identifier -eq "Cove Recovery Testing" ) { Update-CWMAgreementAddition @Update -Value $CDPusage.ContinuityQty | out-null } 
                if ($Addition.product.identifier -eq "Cove Virtual Server" ) { Update-CWMAgreementAddition @Update -Value $CDPusage.VirtServerQty | out-null }  
                if ($Addition.product.identifier -eq "Cove Workstation" ) { Update-CWMAgreementAddition @Update -Value $CDPusage.WorkstationQty | out-null }  
                if ($Addition.product.identifier -eq "Cove Documents" ) { Update-CWMAgreementAddition @Update -Value $CDPusage.DocumentsQty | out-null }  
                if ($Addition.product.identifier -eq "Cove Server" ) { Update-CWMAgreementAddition @Update -Value $CDPusage.PhysServerQty | out-null }  
                if ($Addition.product.identifier -eq "Cove M365" ) { Update-CWMAgreementAddition @Update -Value $CDPusage.M365UserQty | out-null }
                }
                
            start-sleep -seconds 5
            $Additions = Get-CWMAgreementAddition -AgreementID $Agreement.id -all
            $Additions | Out-GridView -Title "$($CDPusage.partnername) >>> $($CWMcompany.name) | company.id $($CWMcompany.id) | agreement.id $($Agreement.id) | Addition IDs & QTYs after updates"
        }
            ...
            if ($CWMcompany) { 
                ...
                Write-Warning "NO MATCH FOUND for Cove Partner [$($CDPusage.PartnerName)] LegalName [$($CDPusage.LegalName)] Reference [$($CDPusage.PartnerRef)] ID [$($CDPusage.PartnerID)]"
            }
        ...
        $Usage = $MVplus | Select-Object Subdisti,Reseller,Reseller_Legal,Reseller_Ref,Reseller_Id,Endcustomer,EndCustomer_legal,Endcustomer_Ref,EndCustomer_id,PhysicalServers,VirtualServer,Workstation,Documents,RecoveryTesting,O365Users | Sort-Object endcustomer -Unique 
      
      
        foreach ($endcustomer in $Usage) {
            
            $CDPusage =  @()    

            $CDPusage += New-Object -TypeName PSObject -Property @{
                PartnerID       = $endcustomer.endcustomer_id ;
                Partnername     = $endcustomer.endcustomer ;
                LegalName       = $endcustomer.endcustomer_Legal ;
                PartnerRef      = $endcustomer.endcustomer_Ref ;

                PhysServerQty   = if ($null -ne (($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.physicalserver -ge 1)} |  measure-object Physicalserver -sum).sum)) 
                                    {($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.Physicalserver -ge 1)} |  measure-object Physicalserver -sum).sum}else{0} ;
                VirtServerQty   = if ($null -ne (($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.virtualserver -ge 1)} |  measure-object virtualserver -sum).sum)) 
                                    {($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.virtualserver -ge 1)} |  measure-object virtualserver -sum).sum}else{0} ;
                WorkstationQty  = if ($null -ne (($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.workstation -ge 1)} |  measure-object workstation -sum).sum)) 
                                    {($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.workstation -ge 1)} |  measure-object workstation -sum).sum}else{0} ;
                DocumentsQty    = if ($null -ne (($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.documents -ge 1)} |  measure-object documents -sum).sum)) 
                                    {($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.documents -ge 1)} |  measure-object documents -sum).sum}else{0} ;
                M365UserQty     = if ($null -ne (($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.o365users -ge 1)} |  measure-object O365Users -sum).sum)) 
                                    {($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.o365users -ge 1)} |  measure-object O365Users -sum).sum}else{0} ;
                ContinuityQty   = IF ($null -ne (($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.RecoveryTesting -ge 1)}).count))
                                    {($mvplus | Where-Object {($_.endcustomer -eq $endcustomer.endcustomer) -and ($_.RecoveryTesting -ge 1)}).count}else{0} ;
            }    

      <#
            if ("" -eq $CDPusage.PhysServerQty) {$CDPusage.PhysServerQty = 0 }
            if ("" -eq $CDPusage.VirtServerQty) {$CDPusage.VirtServerQty = 0 }
            if ("" -eq $CDPusage.WorkstationQty) {$CDPusage.WorkstationQty = 0 }
            if ("" -eq $CDPusage.DocumentsQty) {$CDPusage.DocumentsQty = 0 }
            if (""-eq $CDPusage.M365UserQty) {$CDPusage.M365UserQty = 0 }
            if ("" -eq $CDPusage.ContinuityQty) {$CDPusage.ContinuityQty = 0 }

#>          
            Write-output "`nCove Data Protection High Water Mark Usage for Month ending $period"
