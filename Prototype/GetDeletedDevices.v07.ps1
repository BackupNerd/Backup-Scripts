...
#region ----- Backup.Management JSON Calls ----
    ...
    Function Set-ScriptLocation {
        ...
        # ------------------------------------------------------------#>
            ...
            $STAct = New-ScheduledTaskAction -Execute Powershell -WorkingDirectory 'C:\ProgramData\BackupNerdScripts\GetDeletedDevices' -argument "GetDeletedDevices.v05.ps1 -sendmail -allsubpartners -name 'HEH Computing IT'"
            $STPrin = New-ScheduledTaskPrincipal -UserId $env:USERDOMAIN\$env:USERNAME -LogonType Password -RunLevel Highest
            ...
            Register-ScheduledTask "N-able Backup\GetDeletedDevices" -Action $STAct -Trigger $STTrig
