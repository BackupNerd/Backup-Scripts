...
<# ----- About: ----
    # Deploy N-able Backup Manager In VSA
    # Revision v24.The20LLC - 2023-05-12
...
<# ----- Behavior: ----
    ...
    # Default download to c:\kworking    
    #
...
[CmdletBinding(DefaultParameterSetName="AutoDeploy")]
    Param (
        ...
        [parameter(ParameterSetName="AutoDeploy", Mandatory=$false, Position=0)]
            [parameter(ParameterSetName="Documents", Mandatory=$true, Position=0)] [ValidateLength(36,36)] [string]$Uid = "52a304d8-a2eb-489a-9168-8845e0a846dc",  ## Mandatory Customer UID - Found @ https://Backup.Management | Customer Management 
    ...
    Function Download-BackupManager {
        ...
        (New-Object System.Net.WebClient).DownloadFile("http://cdn.cloudbackup.management/maxdownloads/mxb-windows-x86_x64.exe","c:\kworking\mxb-windows-x86_x64.exe")
    }
    ...
    Function Autodeploy-Passphrase {
        ...
        if (((Test-Path $BMConfig -PathType leaf) -eq $false) -or ($Force)) {
            ...
            if (($ProfileName -eq "") -or ($ProfileName -eq $null)) {$BackupProfile = "-profile-id `"0`"" } else{ $BackupProfile = "-profile-name `"$ProfileName`"" }

            if ($ProfileId) { $BackupProfile = "-profile-id `"$ProfileId`"" }

            #if (($ProductName -ne "") -and ($null -ne $ProductName)) { $BackupProduct = "-product-name `"$ProductName`"" } else { $BackupProduct = "-product-name `"All-In - [365 Days]`"" } 
            
            if (($ProductName -eq "") -or ($null -eq $ProductName)) { $BackupProduct = "-product-name `"All-In - [365 Days]`"" } else { $BackupProduct = "-product-name `"$ProductName`"" } 
    ...
    Function Reuse-BackupConfig {
        ...
        $BMConfig = "C:\Program Files\Backup Manager\config.ini"
        $BMConfigCopy = "C:\programdata\MXB\config.ini.copy2"
