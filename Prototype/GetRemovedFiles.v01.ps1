...
Function Get-RemovedFiles ($starttime) {
    ...
    if ($FileList) {
        ...
        $RemovedFileList = Import-Csv TempFileList.csv
        $RemovedFileList | foreach-object {$_.start = $starttime} 
        $RemovedFileList | Select-Object START,@{Name = 'Present';Expression = { test-path $_.Path }},PATH | Export-Csv -Path RemovedFileList.csv -NoTypeInformation -Append
