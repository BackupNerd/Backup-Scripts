...
Function Get-TimeStamp.retired {
...
$global:ScriptLog = Join-Path -Path $True_path -ChildPath $ScriptBase | Join-Path -ChildPath "$ScriptBase.log"
$Script:Scriptlogsize = Format-FileSize((Get-Item $Scriptlog).length)
$Script:ScriptLogParent = Split-path -path $ScriptLog
md -Force $Script:ScriptLogParent
...
# (No error)
        ...
        $Script:EnumeratePartnersSessionResults = $EnumeratePartnersSession.result.result | select-object Name,@{l='Id';e={($_.Id).tostring()}},Level
        $Script:EnumeratePartnersSessionResults += @( [pscustomobject]@{Name=$PartnerName;Id=[string]$PartnerId;Level='<ParentPartner>'} )

        $Script:SelectedPartner = $EnumeratePartnersSessionResults | Sort-object Level,Name | Out-Gridview -Title "Select a PARTNER from available PARTNERS to list available PROFILES" -OutputMode Single | 
    ...
    Function EnumerateAccountProfiles {
...
# (No error)
        ...
        $Script:SelectedProfile = $EnumerateAccountProfilesSessionResults | where-object {$_.id -ne "5038"} | select-object Name,@{l='Id';e={($_.Id).tostring()}} | Out-Gridview -Title "Select a PROFILE from available PROFILES to later apply to selected DEVICES" -OutputMode Single | Sort-Object name
    ...
    Function EnumerateAccountStatistics {

# ----- Get Devices via EnumerateAccountStatistics -----

    $Filter = "(!(AR =~ '001???????????????- Recycle Bin') AND (OP != 'Documents') AND (AT != 2))"
    $Columns = ('AU','AR','AN','OP','OI','OT','PF','PN','AT','MN','TS','AA843')
    $MaxDeviceCount=2000
...
# (Summarize DeviceDetail)
    ...
    $Script:SelectedDevice = $DeviceDetail | Select-Object Account,PartnerId,Reference,PartnerName,DeviceName,MachineName,AccountID,Product,AccountType,profileId,Profile,OsType,Timestamp,Notes | Out-Gridview -Title "Assign PROFILE | $($selectedProfile.name) | to selected DEVICES | use SHIFT to select multiple DEVICES" -OutputMode Multiple
