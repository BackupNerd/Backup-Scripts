...
<# ----- Behavior: ----
    ...
    # Authenticate
    # Use the -Action "FSSentSize" (Default) Parameter to Get a list of the largest transfered files from the most recent File System backup session
    # Use the -Action "ESXCount" Parameter to Get ESX VMs selected / protected during the most recent VMware/ESX Host backup session
    # Use the -Action "HVCount" Parameter to Get Hyper-V VMs selected / protected during the most recent Hyper-V backup session
    # Use the -Action "VHDinFS" Parameter to Get a list of VHD files that were selected / protetected during the most recent File System backup session
    # Use the -DeviceCount ## (Default=20) Parameter to define how many devices to process
    # Use the -DeviceCount ## (Default=50) Parameter to define how many Files to process per device
    # Use the -GridView Switch Parameter to Display output via Powershell
    ...
    # Use the -ClearCredentials Switch Parameter to wipe stored encrypted credentials
    #
    # Default Values can be changed in the Parameter section below:
    #
    # -----------------------------------------------------------#>  ## Behavior
...
[CmdletBinding()]
    Param (
        [Parameter(Mandatory=$False)] $Action = "FSSent",                       ## Change default Action to get FSSent,FSSize,ESXCount,HVCount,VHDinFS 
...
#region ----- Environment, Variables, Names and Paths ----
    Clear-Host
    Write-output "  Get Session File details "
    Write-output ""
...
Function Send-GetPartnerInfo ($PartnerName) { 
    ...
    Function Get-Devices {
        ...
        ForEach ( $DeviceResult in $Devices.result.result ) {
            ...
            if (($Script:Session.count -ge 1) -and($sessionId)) {
                ...
                #$Script:cleanresponse = $null   ## clears prior query data incase of GateWay Timeout
    ...
    Function Get-ProcessedFiles($DeviceId) {
        ...
        $Script:csvoutputfile = "$PSScriptRoot\$($CurrentDate)_$($action)_Files_$($Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($PartnerId).csv"
