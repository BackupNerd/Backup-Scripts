...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$False)] [switch]$AllDevices ,                         ## Skip GUI device selection
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function Send-GetDevices {
        ...
        ForEach ( $DeviceResult in $DeviceResponse.result.result ) {
            ...
            #$AccountID
            #$Script:AccountInfoResponse.result.result.Token
    ...
    Function GetM365Stats {
        ...
        $script:devicestatistics = $M365response.deviceStatistics | Select-object @{N="Partner";E={$device.partnername}},
                                                                                    ...
                                                                                    @{N="Mail";E={$_.datasources.status[0] -replace("unprotected","Excluded") -replace("protected","Selected") }},
                                                                                    @{N="OneDrive";E={$_.datasources.status[1] -replace("unprotected","Excluded") -replace("protected","Selected")}},
                                                                                    @{N="SharePoint";E={$_.datasources.status[2] -replace("unprotected","Excluded") -replace("protected","Selected")}},
                                                                                    userid 
    ...
    Function EnumerateM365Users ($AccountToken) {
        ...
        $script:M365UserStatistics = $EnumerateM365UsersResponse.result.result.Users | Select-object @{N="Partner";E={$device.partnername}},
                                                                                                    ...
                                                                                                    @{N="MailBoxSelectedGiB_notused";E={[math]::Round([Decimal](($_.ExchangeInfo.SelectedSize) /1GB),3) }},
    ...
    foreach ($device in $SelectedDevices) {
        ...
        If ($Script:Exportcombined) {
            $Script:csvoutputfile2a = "$ExportPath\$($CurrentDate)_M365_User_Summary_$($Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($PartnerId).csv"
        ...
        If ($Script:Exportcombined) {
            $Script:csvoutputfile2b = "$ExportPath\$($CurrentDate)_M365_User_Detail_$($Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($PartnerId).csv"
        ...
        If ($Script:ExportIndividual) {
            $Script:csvoutputfile3a = "$ExportPath\$($CurrentDate)_M365_User_Summary_$($device.Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($device.PartnerId).csv"
        ...
        If ($Script:ExportIndividual) {
            $Script:csvoutputfile3b = "$ExportPath\$($CurrentDate)_M365_User_Detail_$($device.Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($device.PartnerId).csv"
