...
<# ----- Behavior: ----
    # Script for bulk file-level recovery from a Cove backup using only the local Backup Manager
    # via script or command line -- internet required.
    #
    # Originally designed for machines being decommissioned or already offboarded where you need
    # to pull data out before the backup account is fully removed. Also useful for any scenario
    # requiring unattended multi-session restores.
    #
    # Primary use cases:
    #   - Bulk historical restores: restore multiple sessions in sequence (weekly Fridays,
    #     end-of-month, etc.) unattended overnight without manually triggering each one via GUI.
    #   - Forensic/compliance restores: recover specific point-in-time versions (Daily/Weekly/Monthly
    #     filters) from a file path affected by ransomware or accidental deletion.
    #   - Scripted/automated restore workflows: integrate into a recovery runbook where an operator
    #     selects sessions from a GridView, then walks away.
    #
    # Why not use the GUI?
    #   - GUI does one session at a time with no automation.
    #   - This script handles queuing, progress monitoring, logging, and skip-on-failure automatically
    #
    # Performance tips:
    #   - Consider adding a local SpeedVault as the restore source for faster throughput
    #   - Add RestoreDownloadThreadCount=50 to the [General] section of config.ini and restart
    #     the BackupFP service to increase parallel download threads during restore
    #   - Consider using a Restore-Only installation of the Backup Manager on the source machine
    #
    # Behavior:
    ...
    #                                    Waits for engine to reach Idle before starting a restore.
    #                                    Required because ClientTool rejects restore.start if a backup or restore is already running.
    #                                    Skips the session and moves on if Idle is not reached within 120 minutes.
    #
    ...
    #                                    Two independent signals are checked each tick -- either one is sufficient:
    #                                      1. ClientTool status = "Restore"   -> long-running restore still in progress
    #                                      2. SessionReport.xml new Restore record (Plugin=$pluginName, Type=Restore, Id>snapshotId)
    #                                         -> fast/sub-second restore already complete before first status poll
    #                                    Skips the session if neither signal appears within 60 seconds.
    #
    #   - Restore completion wait:        Unbounded   (SessionReport.xml poll every 5s; exits on session record)
    #                                    Only entered when Phase 1 confirmed via ClientTool status (restore still running).
    #                                    Polls SessionReport.xml until a new session record appears (written at restore end).
    #                                    No timeout -- waits as long as needed; restore may take hours for large datasets.
    #
    # Parameters:
...
.NOTES
    Author:  Eric Harless, Head Backup Nerd - N-able
    Twitter: @Backup_Nerd  Email: eric.harless@n-able.com
    Version: 12.2 - 2026-07-07
#>

param (
        [string[]]$RestoreSelection = @("D:\data1","d:\data2"),                      ## Path(s) to restore; multiple paths each get their own -selection argument
        [string]$RestoreBase = "D:\Restore\",                                        ## Base directory where the restored files will be placed 
        [switch]$CombinedRestore = $true,                                            ## ( if $true ) Combine multiple date restores into a single folder, else dated folders
