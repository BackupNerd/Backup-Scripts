...
Param (
    ...
    [Parameter(Mandatory=$False)][Switch]$CombineServers,                             ## Combine Physical + Virtual server qtys into a single CWM addition (CombinedServerQty). When $true, Phys/Virt additions are zeroed.
    ...
    [Parameter(Mandatory=$False)][int]$PhysServerFUGB = 2000,                                   ## Selected Size Fair Use / Included GB, 0 to pass full value
    [Parameter(Mandatory=$False)][int]$VirtServerFUGB = 2000,                                   ## Selected Size Fair Use / Included GB, 0 to pass full value
    [Parameter(Mandatory=$False)][int]$WorkstationFUGB = 500,                                  ## Selected Size Fair Use / Included GB, 0 to pass full value
    
    [Parameter(Mandatory=$False)][ValidateSet("Name","Type")][String]$CWMAgreementBehavior = "Name",  ## Agreement lookup mode: 'Name' matches by CWMAgreementName; 'Type' matches by agreement type names in CWMAgreementTypes
    [Parameter(Mandatory=$False)][String]$CWMAgreementName = "Monthly Agreement",            ## ConnectWise Manage Agreement Name (Preferred = 'CoveDataProtection')
    [Parameter(Mandatory=$False)][String[]]$CWMAgreementTypes = @("Co-Managed Basic","Co-Managed Elevated","Legacy Time & Materials","Managed Flat Rate (Worry Free)"),  ## Agreement type names to match when CWMAgreementBehavior = 'Type'
      
    [Parameter(Mandatory=$False)][String]$CWMAgreementSearchType = "Contains",                  ## ConnectWise Manage Agreement Search Type. Can be either Equals, Contains, StartsWith, EndsWith
    [Parameter(Mandatory=$False)][String]$CWMAdditionSearchType = "StartsWith",                   ## ConnectWise Manage Addition Search Type. Can be either Equals or StartsWith
    [Parameter(Mandatory=$False)][System.Nullable[bool]]$SetProrateFlag = $null,               ## Set prorateCurrentPeriodFlag on Cove-mapped additions before qty update: $false=suppress prorate, $true=enable, $null=skip (default)
    ...
    [Parameter(Mandatory=$False)][Switch]$TrialState = $true,                                ## Treat partners in trial state as Billable when set $true
    [Parameter(Mandatory=$False)][Switch]$IgnoreDeletedDevices = $false,                       ## Exclude devices with a DeviceDeletionDate from quantities and CWM totals
...
$CDP2CWMProductMapping = @{
    ...
    "PhysicalServerQty"     = "CDP-Physical-Server"    ## ConnectWise Manage Product ID (Recommended Product ID = 'CDP-Physical-Servers', Product Description = 'Cove Data Protection - Physical Server Backup')
    "VirtualServerQty"      = "CDP-Virtual-Server"     ## ConnectWise Manage Product ID (Recommended Product ID = 'CDP-Virtual-Servers', Product Description = 'Cove Data Protection - Virtual Server Backup')
    "CombinedServerQty"     = "CDP-Combined-Servers"    ## ConnectWise Manage Product ID (Recommended Product ID = 'CDP-Total-Servers', Product Description = 'Cove Data Protection - Total Server Backup')
    "WorkstationQty"        = "CDP-Workstation"        ## ConnectWise Manage Product ID (Recommended Product ID = 'CDP-Workstations', Product Description = 'Cove Data Protection - Workstation Professional Backup')
    "DocumentsQty"          = "CDP-Document"           ## ConnectWise Manage Product ID (Recommended Product ID = 'CDP-Documents', Product Description = 'Cove Data Protection - Workstation Documents Backup')
    "O365UsersQty"          = "CDP-M365-User"          ## ConnectWise Manage Product ID (Recommended Product ID = 'CDP-M365-Users', Product Description = 'Cove Data Protection - M365 User Backup')
    ...
    "CoveGBSelected"        = "CDP-Overage-GB"     ## ConnectWise Manage Product ID (Recommended Product ID = 'CDP-GBSelected-Size', Product Description = 'Cove Data Protection - GB Selected Size')
...
Function Display-CurrentParameters ($ParamLabel) {
    $Parameters = [ordered]@{
        ...
        DebugCDP                = $DebugCDP
        DebugCWM                = $DebugCWM
        SetProrateFlag          = if ($null -eq $SetProrateFlag) { '(not set)' } else { $SetProrateFlag }
    }
...
$Script:CoveAuditLogFile = "$CurrentExportPath\$($CurrentDate)_$($Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($PartnerId)_CoveAuditLog.csv"

$_auditQtyHeader = if ($NoCWMUpdate -or -not $ShouldProcessCWMAdditions) { "Proposed Quantity" } else { "Updated Quantity" }
Add-content -Path $AuditLogFile -Value "Company Id,Company Name,Agreement Name,Agreement Type,Additions Product ID,Additions Description,Effective Date,Additions Quantity,$_auditQtyHeader"
Add-content -Path $CoveAuditLogFile -Value "Company Id,Company Name,Agreement Name,Agreement Type,Additions Product ID,Additions Description,Effective Date,Additions Quantity,$_auditQtyHeader"

## Writes each audit row to the main log, and also to the Cove-only log when the Additions Description starts with "Cove"
function Write-AuditLogLine {
    param([string]$Description, [string]$Value)
    Add-Content -Path $AuditLogFile -Value $Value
    if ($Description -like 'Cove*') { Add-Content -Path $CoveAuditLogFile -Value $Value }
}
...
## Real caught exceptions (API/network failures) go here; expected not-found/skip cases stay in ExceptionsLogFile
$Script:ErrorsLogFile = "$CurrentExportPath\$($CurrentDate)_$($Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($PartnerId)_ErrorsLog.txt"
...
if ($GetCDPUsage) {
    ...
    $Script:MVPlus | foreach-object { if (-not $_.OsType) {$_.FairUseGB = $PhysServerFUGB }}
    ...
    $Script:MVPlus | foreach-object { if (($_.OsType -eq "Server") -or ($_.PhysicalServer -eq 1) -or ($_.VirtualServer -eq 1) -or ($_.Workstation -eq 1) -or (-not $_.OsType) -or (($null -eq $_.Documents) -and ($_.O365Users -eq 0))) {[decimal]$_.OverageGB = ([decimal]$_.SelectedSizeGB - [decimal]$_.FairuseGB) }}
    ...
    foreach ($customer in $Usage) {
        ...
        ## Select-Object rejects a $null property (returned by the if-expression when CombineServers=$false), so build the list conditionally
        $_cdpUsageProps = @('SubDisti','Reseller','SO','EndCustomer','PartnerID','Partnername','LegalName','PartnerRef')
        if ($CombineServers) { $_cdpUsageProps += 'CombinedServerQty' }
        $_cdpUsageProps += @('PhysicalServerQty','VirtualServerQty','WorkstationQty','DocumentsQty','RecoveryTestingQty','O365UsersQty',@{n='CoveGBSelected';e={[int]$_.CoveGBSelected}})

        $CDPusage | Select-Object $_cdpUsageProps |
...
if ($ConnectCWM) {
            ...
            for ($attempt = 1; $attempt -le 2; $attempt++) {
                try {
                    ...
                    if ($attempt -lt 2) {
                        ...
                        Add-content -Path $ErrorsLogFile -Value "CWM agreement query failed after 2 attempts for [$QueryLabel]. Error: $($_.Exception.Message)"
                        return $null
                    }
                }
            }
        ...
        if ($CWMcompany) {
            ...
            if ($CWMAgreementBehavior -eq "Type") {
                ...
                if ($null -eq $CWMAgreement -or $CWMAgreement.Count -eq 0) {
                    ...
                    UpdateCWMQty    ## Read current additions; write only if ShouldProcessCWMAdditions=$true and NoCWMUpdate=$false
                }
            } else {
                ## Name mode (original behavior): look for active agreement matching CWMAgreementName
                if ($CWMAgreementSearchType -eq "Contains") {
                    [array]$CWMAgreement = Get-CWMAgreement -Condition "company/id=$($CWMcompany.id) AND name contains `"$CWMAgreementName`" and agreementStatus = `"Active`""    ## Contains $CWMAgreementName
                } elseif ($CWMAgreementSearchType -eq "StartsWith") {
                    [array]$CWMAgreement = Get-CWMAgreement -Condition "company/id=$($CWMcompany.id) AND name like `"$CWMAgreementName%`" and agreementStatus = `"Active`""       ## StartsWith $CWMAgreementName
                } elseif ($CWMAgreementSearchType -eq "EndsWith") {
                    [array]$CWMAgreement = Get-CWMAgreement -Condition "company/id=$($CWMcompany.id) AND name like `"%$CWMAgreementName`" and agreementStatus = `"Active`""       ## EndsWith $CWMAgreementName
                } else {
                    [array]$CWMAgreement = Get-CWMAgreement -Condition "company/id=$($CWMcompany.id) AND name = `"$CWMAgreementName`" and agreementStatus = `"Active`""           ## Matches $CWMAgreementName
                }
                ## An array with >1 match must be resolved to a single agreement before UpdateCWMQty -- otherwise
                ## $CWMAgreement.id/.name become space-joined arrays, producing an invalid additions URL (404).
                if ($null -eq $CWMAgreement -or $CWMAgreement.Count -eq 0) {
                    ...
                    Write-Host "  [i] Multiple ($($CWMAgreement.Count)) agreements matching [$CWMAgreementName] for [$($CWMcompany.Name)]:" -ForegroundColor Cyan
                    $CWMAgreement | ForEach-Object {
                        Write-Host ("      [{0,6}]  {1,-40}  type: {2}" -f $_.id, $_.name, $_.type.name) -ForegroundColor DarkCyan
                    }
                    Write-Host "  [!] Multiple ($($CWMAgreement.Count)) matching agreements found for: [$($CWMcompany.Name)] -- skipping update" -ForegroundColor Yellow
                    Add-content -Path $ExceptionsLogFile -Value "Multiple ($($CWMAgreement.Count)) active agreements matching Agreement Name: [$CWMAgreementName] found for ConnectWise Client Name: [$($CWMcompany.Name)]. Cannot determine which to update -- skipping."
                } else {
                    $CWMAgreement = $CWMAgreement[0]
                    Write-Host "  [+] Agreement [$($CWMAgreement.name)] found for: [$($CWMcompany.Name)]" -ForegroundColor Green
                    UpdateCWMQty    ## Read current additions; write only if ShouldProcessCWMAdditions=$true and NoCWMUpdate=$false
                }
            }
        }
    ...
    Function UpdateCWMQty {
        $ReadOnlyCWMUsageMode = ($NoCWMUpdate -or -not $ShouldProcessCWMAdditions)

        try {
            ## cancelledDate = null excludes additions the customer has cancelled but that still exist on the agreement
            $CWMAdditions = Get-CWMAgreementAddition -AgreementID $CWMAgreement.id -Condition "agreementStatus = `"Active`" and cancelledDate = null" -all -ErrorAction Stop
        } catch {
            ...
            Add-content -Path $ErrorsLogFile -Value "Failed to retrieve CWM agreement additions for ConnectWise Client Name: [$($CWMcompany.Name)] Agreement: [$($CWMAgreement.name)]. Error: $($_.Exception.Message)"
            return
        }
        ...
        if ($debugCWM -and -not $ReadOnlyCWMUsageMode) {
        ...
        if ($ReadOnlyCWMUsageMode) {
            # Build a lookup of proposed quantities using the same matching logic as the update path
            $proposedQty = @{}
            foreach ($add in $CWMAdditions) {
                $id = $add.product.identifier
                foreach ($entry in $CDP2CWMProductMapping.GetEnumerator()) {
                    if ($id.$CWMAdditionSearchType($entry.Value)) {
                        $cdpProp = $entry.Key
                        if ($CombineServers -and $cdpProp -in @('PhysicalServerQty','VirtualServerQty')) {
                            $proposedQty[$id] = [decimal]0
                        } elseif (-not $CombineServers -and $cdpProp -eq 'CombinedServerQty') {
                            # Not applicable in non-combine mode -- skip
                        } else {
                            $val = $CDPusage.$cdpProp
                            $proposedQty[$id] = if ($null -ne $val) { [decimal]$val } else { [decimal]0 }
                        }
                        break
                    }
                }
            ...
            Write-Host ''
            Write-Host ("  {0}  |  {1}  |  {2}" -f $CDPusage.Partnername, $CWMcompany.name, $CWMAgreement.name) -ForegroundColor Cyan
            Write-Host '  Current vs Proposed (read-only -- no update applied):' -ForegroundColor White
            Write-Host ("  {0,-$maxIdLen}  {1,8}  {2,8}  {3,8}" -f 'Product', 'Current', 'Proposed', 'Change') -ForegroundColor White
            Write-Host ("  {0}  {1}  {2}  {3}" -f ('-' * $maxIdLen), '--------', '--------', '--------') -ForegroundColor DarkGray
            $CWMAdditions | Sort-Object { $_.product.identifier } | ForEach-Object {
                $id      = $_.product.identifier
                $current = [decimal]$_.quantity
                $isCove  = $proposedQty.ContainsKey($id)
                $productLabel = if ($isCove) { "[C] $id" } else { "    $id" }
                if ($isCove) {
                    $proposed = $proposedQty[$id]
                    $diff = $proposed - $current
                    if     ($diff -gt 0) { $changeStr = "+{0:N2}" -f $diff; $rowColor = 'Green'  }
                    elseif ($diff -lt 0) { $changeStr = "{0:N2}"  -f $diff; $rowColor = 'Red'    }
                    else                 { $changeStr = '+0.00';              $rowColor = 'Yellow' }
                    Write-Host ("  {0,-$maxIdLen}  {1,8:N2}  {2,8:N2}  {3,8}" -f $productLabel, $current, $proposed, $changeStr) -ForegroundColor $rowColor
                    Write-AuditLogLine -Description $_.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($_.product.identifier)`",`"$($_.description)`",`"$(if ($_.effectiveDate) { ([datetime]$_.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$current),$proposed"
                } else {
                    Write-Host ("  {0,-$maxIdLen}  {1,8:N2}  {2,8}  {3,8}" -f $productLabel, $current, '(n/a)', '') -ForegroundColor Gray
                    Write-AuditLogLine -Description $_.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($_.product.identifier)`",`"$($_.description)`",`"$(if ($_.effectiveDate) { ([datetime]$_.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$current),`"`""
                }
            }
            Write-Host ''
            Write-Host '  [NoCWMUpdate] Read-only snapshot complete; no updates were applied.' -ForegroundColor DarkCyan
            return
        }
        ...
        foreach ($CWMAddition in $CWMAdditions) {
            ...
            if ($($CWMAddition.product.identifier).$CWMAdditionSearchType($($CDP2CWMProductMapping.PhysicalServerQty))) {
                if ($CombineServers) {
                    ...
                    Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),0"
                    if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
                    ...
                    Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),$($CDPusage.PhysicalServerQty)"
                    if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
            ...
            if ($($CWMAddition.product.identifier).$CWMAdditionSearchType($($CDP2CWMProductMapping.VirtualServerQty))) {
                if ($CombineServers) {
                    ...
                    Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),$($CDPusage.VirtualServerQty)"
                    if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
            ...
            if ($($CWMAddition.product.identifier).$CWMAdditionSearchType($($CDP2CWMProductMapping.CombinedServerQty))) {
                if ($CombineServers) {
                    ...
                    Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),$($CDPusage.CombinedServerQty)"
                    if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
            ...
            if ($($CWMAddition.product.identifier).$CWMAdditionSearchType($($CDP2CWMProductMapping.WorkstationQty))) {
                ...
                Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),$($CDPusage.WorkstationQty)"
                if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
            ...
            if ($($CWMAddition.product.identifier).$CWMAdditionSearchType($($CDP2CWMProductMapping.DocumentsQty))) {
                ...
                Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),$($CDPusage.DocumentsQty)"
                if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
            ...
            if ($($CWMAddition.product.identifier).$CWMAdditionSearchType($($CDP2CWMProductMapping.RecoveryTestingQty))) {
                ...
                Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),$($CDPusage.RecoveryTestingQty)"
                if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
            ...
            if ($($CWMAddition.product.identifier).$CWMAdditionSearchType($($CDP2CWMProductMapping.O365UsersQty))) {
                ...
                Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),$($CDPusage.O365UsersQty)"
                if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
            ...
            if ($($CWMAddition.product.identifier).$CWMAdditionSearchType($($CDP2CWMProductMapping.CoveGBSelected))) {
                ...
                Write-AuditLogLine -Description $CWMAddition.description -Value "`"$($CWMcompany.id)`",`"$($CWMcompany.name)`",`"$($CWMAgreement.name)`",`"$($CWMAgreement.type.name)`",`"$($CWMAddition.product.identifier)`",`"$($CWMAddition.description)`",`"$(if ($CWMAddition.effectiveDate) { ([datetime]$CWMAddition.effectiveDate).ToString('yyyy-MM-dd') } else { '' })`",$([Int]$CWMAddition.quantity),$($CDPusage.CoveGBSelected)"
                if ($null -ne $SetProrateFlag) { Update-CWMAgreementAddition @ProrateUpdate -Value $SetProrateFlag | Out-Null }
        ...
        if ($debugCWM) {
            ...
            $CWMAdditions = Get-CWMAgreementAddition -AgreementID $CWMAgreement.id -Condition "agreementStatus = `"Active`" and cancelledDate = null" -all
            $maxIdLen = [Math]::Max(28, ($CWMAdditions | ForEach-Object { ("[C] " + $_.product.identifier).Length } | Measure-Object -Maximum).Maximum)
            Write-Host ""
            Write-Host ("  {0}  |  {1}  |  {2}" -f $CDPusage.partnername, $CWMcompany.name, $CWMAgreement.name) -ForegroundColor Cyan
    ...
    if ($LoadCDPUsage) {
        ...
        Foreach ($CDPusage in $SelectedUsage) {
            $fileMonthMatch = [regex]::Match($OpenFileName, '_CDPUsageFile_(\d{4}-\d{2})_')
            $fileMonthEnd   = if ($fileMonthMatch.Success) { (Get-Date "$($fileMonthMatch.Groups[1].Value)-01").AddMonths(1).AddDays(-1).ToString('yyyy-MM-dd') } else { (Get-Date $Period -Day 1).AddMonths(1).AddDays(-1).ToString('yyyy-MM-dd') }
            Write-Output "`nCove Data Protection Usage Import from File for Month ending $fileMonthEnd"
...
Write-Output "  Cove Audit Log File: $CoveAuditLogFile"
Write-Output "  Exceptions Log File: $ExceptionsLogFile"
Write-Output "  Errors Log File: $ErrorsLogFile"
