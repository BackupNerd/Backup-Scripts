            ...
            try {
                ...
                if ($PSCmdlet.ShouldProcess($MonitoringScriptPath, "Create backup and update configuration")) {
                    ...
                    if ($Script:SelectedConfig.Board) {
                        $scriptContent = $scriptContent -replace '(\[Parameter\(Mandatory=\$False\)\]\[string\]\$TicketBoard\s*=\s*")[^"]*(")', "`${1}$($Script:SelectedConfig.Board)`$2"
                    }
                    if ($Script:SelectedConfig.NewStatus) {
                        $scriptContent = $scriptContent -replace '(\[Parameter\(Mandatory=\$False\)\]\[string\]\$TicketStatus\s*=\s*")[^"]*(")', "`${1}$($Script:SelectedConfig.NewStatus)`$2"
                    }
                    if ($Script:SelectedConfig.ClosedStatus) {
                        $scriptContent = $scriptContent -replace '(\[Parameter\(Mandatory=\$False\)\]\[string\]\$TicketClosedStatus\s*=\s*")[^"]*(")', "`${1}$($Script:SelectedConfig.ClosedStatus)`$2"
                    }
                    if ($Script:SelectedConfig.PriorityServer) {
                        $scriptContent = $scriptContent -replace '(\[Parameter\(Mandatory=\$False\)\]\[string\]\$TicketPriorityServer\s*=\s*")[^"]*(")', "`${1}$($Script:SelectedConfig.PriorityServer)`$2"
                    }
                    if ($Script:SelectedConfig.PriorityWorkstation) {
                        $scriptContent = $scriptContent -replace '(\[Parameter\(Mandatory=\$False\)\]\[string\]\$TicketPriorityWorkstation\s*=\s*")[^"]*(")', "`${1}$($Script:SelectedConfig.PriorityWorkstation)`$2"
                    }
                    if ($Script:SelectedConfig.PriorityM365) {
                        $scriptContent = $scriptContent -replace '(\[Parameter\(Mandatory=\$False\)\]\[string\]\$TicketPriorityM365\s*=\s*")[^"]*(")', "`${1}$($Script:SelectedConfig.PriorityM365)`$2"
                    }
                    ...
                    Write-Host "`n  To undo this change, run: .\Cove2CWM-SetTicketsConfig.ps1 -Undo" -ForegroundColor Cyan
