<# ----- About: ----
    ...
    # Revision v10.4 - 2026-07-08 - Documents Profile Support & Machine Name in Subjects
    # Author: Eric Harless, Head Backup Nerd - N-able
    # Twitter @Backup_Nerd  Email:eric.harless@n-able.com
    # https://github.com/backupNerd
    #
    # v10.4 Changes (2026-07-08):
    #   - Added Documents Profile distinction: workstations with specific profile (default 5038) use separate 96h stale threshold
    #   - Ticket naming now includes "COVE DOC" prefix for Documents workstations (vs "COVE WKS" for regular)
    #   - Ticket subjects now display machine names (MN) by default instead of device names (AN)
    #   - Added UseDeviceNameInSubject parameter to toggle between machine name (default) and device name in subjects
    #   - Ticket matching now uses AccountId (globally unique) instead of device name for guaranteed accuracy
    #   - Fixed closure pass to query OI (Profile ID) column and apply correct thresholds for Documents devices
    #   - Fixed closure logic to NOT skip just-created/updated tickets if device is now healthy and within threshold
    #   - Enhanced debug output to show ProfileID extraction and threshold selection in closure pass
    #
    # v10.3 Changes (2026-03-02):
    #   - CDP password now held as SecureString for its entire lifetime (Get-APICredentials)
    #   - CDP password decrypted to plaintext only at point of API POST, cleared immediately after (Send-APICredentialsCookie)
    #   - CWM SecureString intermediate objects (privateKey, pubKey, clientId) now call .Dispose() after use (Get-CWMAPICreds)
    #   - CWM debug output no longer logs Private Key or Public Key in plaintext (AuthenticateCWM)
    #   - New-Item for credential directory now uses -Force | Out-Null (Set-APICredentials)
    #
    # v10.2 Changes (2026-02-18):
    #   - Added automatic ConnectWiseManageAPI module installation from PowerShell Gallery if missing
    #   - Added automatic PowerShell 7 compatibility patch detection and application at runtime
    #   - Script now self-maintains - installs module and applies PS7 fix automatically on first run
    #   - Checks for newer module versions and notifies (manual update required)
    #   - Creates timestamped backups before patching module files
    #   - Reference: https://github.com/christaylorcodes/ConnectWiseManageAPI/issues/70
    #
...
<# ----- Behavior: ----
    ...
    # Use the -StaleHoursWorkstations parameter to define hours since last successful backup before considering stale for workstations (default=25)
    # Use the -StaleHoursDocuments parameter to define hours since last successful backup before considering stale for workstations with Documents profile (default=26)
    # Use the -DocumentsProfileId parameter to specify the Profile ID that identifies workstations with Documents backup profile (default=5038)
    #   Documents workstations are identified by ProfileID and use separate thresholds/naming conventions
    #   Ticket naming: "COVE DOC machinename #AccountId - Issue" (vs "COVE WKS machinename #AccountId - Issue" for regular)
    # 
    ...
    # Use the -UseDeviceNameInSubject parameter to use device name in ticket subject (default=$false uses machine name instead)
    #   Default ($false): "COVE DOC EXT1012C #3785154 - FS Stale" (machine name)
    #   Set to $true:     "COVE DOC ext1012c_6qwxr #3785154 - FS Stale" (device name)
    ...
    # Use the -TicketCompany parameter to specify fallback company name/ID when device company not found in CWM (optional)
    #   TicketCompany only used after all 4 company lookup strategies fail (and auto-create if enabled)
    #   Supports both company name (string) or company ID (integer)
...
.PARAMETER StaleHoursDocuments
Hours since last successful backup before a workstation with Documents profile is considered stale. Default: 96 hours (4 days).

.PARAMETER DocumentsProfileId
Profile ID that identifies workstations configured with Documents backup profile. Workstations matching this profile use StaleHoursDocuments threshold instead of StaleHoursWorkstations. Default: 5038.

.PARAMETER StaleHoursMSSQL
Hours since last successful MSSQL backup before the datasource is considered stale. Checked against the ZL (MSSQL Last Success) column, not the device-level TL. Set to 0 to disable (default). Only devices with D03 configured in I78 are evaluated.
...
Automatically create missing companies in ConnectWise Manage when Cove partner has no CWM match. Set to $false to disable auto-creation. Default: $false.
...
Fallback company name or ID for tickets when Cove device's company cannot be matched in ConnectWise.
Only used after all 4 company lookup strategies fail (and auto-create if enabled).
Supports both company name (string) or company ID (integer).
Leave empty to skip ticket creation when no company match found.
Example: "Default MSP Company" or "12345"
...
Bypass SSL certificate validation. Only use for staging/development environments.
...
Param (
    ...
    [int]$StaleHoursServers = 24,                                 ## Hours since last backup to consider stale (servers)
    [int]$StaleHoursWorkstations = 24,                            ## Hours since last backup to consider stale (workstations)
    [int]$StaleHoursDocuments = 336,                               ## Hours since last backup to consider stale (documents)
    [int]$DocumentsProfileId = 5038,                              ## Profile ID that identifies workstations with Documents profile
    [int]$StaleHoursMSSQL = 0,                                    ## (In progress) Hours since last MSSQL backup to consider stale (0 = disabled)
    ...
    [string]$TicketCompany = "",                                 ## Fallback company name/ID when device company not found in CWM
    [string]$ExportPath = "$PSScriptRoot",                       ## Export Path for CSV

    [bool]$UseDeviceNameInSubject = $false,                       ## Use device name (AN) in ticket subject. Set $false to use machine name (MN)
    ...
    # Testing Commands
    [bool]$AllowInsecureSSL = $false,                             ## Bypass SSL certificate validation (for staging/dev)
...
Clear-Host
$Script:ScriptVersion = "v10.4"
Write-Host "PowerShell Version: $($PSVersionTable.PSVersion)" -ForegroundColor Gray

#region ----- ConnectWiseManageAPI Module Auto-Install & PowerShell 7 Patch ----
Write-Host "`nChecking ConnectWiseManageAPI module..." -ForegroundColor Cyan

# Check if module is installed
$cwmModule = Get-Module ConnectWiseManageAPI -ListAvailable | Sort-Object Version -Descending | Select-Object -First 1

if (-not $cwmModule) {
    Write-Host "  Module not found - installing from PowerShell Gallery..." -ForegroundColor Yellow
    try {
        Install-Module ConnectWiseManageAPI -Scope CurrentUser -Force -AllowClobber -ErrorAction Stop
        $cwmModule = Get-Module ConnectWiseManageAPI -ListAvailable | Select-Object -First 1
        Write-Host "  ✅ Installed: v$($cwmModule.Version)" -ForegroundColor Green
    }
    catch {
    ...
    Write-Host "  ✅ Found: v$($cwmModule.Version)" -ForegroundColor Green
    
    # Check for updates (optional - only check, don't auto-update)
    try {
        $galleryModule = Find-Module ConnectWiseManageAPI -ErrorAction Stop
        if ([version]$galleryModule.Version -gt [version]$cwmModule.Version) {
            Write-Host "  ℹ️  Newer version available: v$($galleryModule.Version) (current: v$($cwmModule.Version))" -ForegroundColor Cyan
            Write-Host "     To update: Update-Module ConnectWiseManageAPI -Force" -ForegroundColor Gray
        }
    }
    catch {
        # Ignore if can't check gallery (offline, etc)
    }
}

# Check if PowerShell 7 patch is needed
$invokeWebRequestFile = Get-ChildItem -Path $cwmModule.ModuleBase -Recurse -Filter "Invoke-CWMWebRequest.ps1" | Select-Object -First 1

if ($invokeWebRequestFile) {
    $fileContent = Get-Content $invokeWebRequestFile.FullName -Raw
    $hasFix = $fileContent -match '-AllowInsecureRedirect'
    
    if (-not $hasFix) {
        Write-Host "  ⚠️  PowerShell 7 fix not present - applying patch..." -ForegroundColor Yellow
        
        # Create backup
        $backupPath = "$($invokeWebRequestFile.FullName).backup.$(Get-Date -Format 'yyyyMMdd_HHmmss')"
        Copy-Item $invokeWebRequestFile.FullName -Destination $backupPath -Force
        Write-Host "     Backup: $(Split-Path $backupPath -Leaf)" -ForegroundColor Gray
        
        # Apply patch - add -AllowInsecureRedirect to both Invoke-WebRequest calls
        $fileContent = $fileContent -replace `
            '(\$Result = Invoke-WebRequest @Arguments -UseBasicParsing)(?!\s+-AllowInsecureRedirect)', `
            '$1 -AllowInsecureRedirect:$true'
        
        # Write patched content
        $fileContent | Set-Content $invokeWebRequestFile.FullName -NoNewline -Force
        
        # Verify patch
        $verifyContent = Get-Content $invokeWebRequestFile.FullName -Raw
        $patchCount = ([regex]::Matches($verifyContent, '-AllowInsecureRedirect')).Count
        
        if ($patchCount -ge 2) {
            Write-Host "  ✅ PowerShell 7 patch applied successfully ($patchCount occurrences)" -ForegroundColor Green
        }
        else {
            Write-Host "  ⚠️  Patch may be incomplete ($patchCount occurrences, expected 2+)" -ForegroundColor Yellow
        }
    }
    else {
        Write-Host "  ✅ PowerShell 7 patch already present" -ForegroundColor Green
    }
}
else {
    Write-Host "  ⚠️  Could not locate Invoke-CWMWebRequest.ps1 for patching" -ForegroundColor Yellow
}
...
#endregion ----- ConnectWiseManageAPI Module Auto-Install & PowerShell 7 Patch ----

#region ----- Helper Scripts Inventory ----
Write-Host "`n╔══════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║                  HELPER SCRIPTS INVENTORY                        ║" -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan

$Script:HelperScripts = @{
    SyncCustomers = @{
        Name = "Cove2CWM-SyncCustomers"
        PreferredVersion = "v10"
        Purpose = "Auto-creates missing companies in ConnectWise Manage"
        RequiredFor = "Auto-Create Companies feature (-AutoCreateCompanies)"
        Found = $false
        Path = $null
        VersionFound = $null
    }
    SetTicketsConfig = @{
        Name = "Cove2CWM-SetTicketsConfig"
        PreferredVersion = "v10"
        Purpose = "Interactive wizard to configure ticket boards/statuses/priorities"
        RequiredFor = "First-time setup or when ticket parameters are missing"
        Found = $false
        Path = $null
        VersionFound = $null
    }
...
# Check for Cove2CWM-SyncCustomers helper script
$syncCustomersPath = Join-Path $PSScriptRoot "Cove2CWM-SyncCustomers.v10.ps1"
if (Test-Path $syncCustomersPath) {
    $Script:HelperScripts.SyncCustomers.Found = $true
    $Script:HelperScripts.SyncCustomers.Path = $syncCustomersPath
    $Script:HelperScripts.SyncCustomers.VersionFound = "v10"
}
else {
    # Try to find any version
    $allVersions = Get-ChildItem -Path $PSScriptRoot -Filter "Cove2CWM-SyncCustomers*.ps1" -File -ErrorAction SilentlyContinue | Sort-Object Name -Descending
    if ($allVersions.Count -gt 0) {
        $Script:HelperScripts.SyncCustomers.Found = $true
        $Script:HelperScripts.SyncCustomers.Path = $allVersions[0].FullName
        # Extract version from filename (e.g., "v08" from "Cove2CWM-SyncCustomers.v08.ps1")
        if ($allVersions[0].Name -match '\.v(\d+)\.ps1$') {
            $Script:HelperScripts.SyncCustomers.VersionFound = "v$($matches[1])"
        }
        else {
            $Script:HelperScripts.SyncCustomers.VersionFound = "unknown"
        }
    }
}

# Check for Cove2CWM-SetTicketsConfig helper script
$setConfigPath = Join-Path $PSScriptRoot "Cove2CWM-SetTicketsConfig.v10.ps1"
if (Test-Path $setConfigPath) {
    $Script:HelperScripts.SetTicketsConfig.Found = $true
    $Script:HelperScripts.SetTicketsConfig.Path = $setConfigPath
    $Script:HelperScripts.SetTicketsConfig.VersionFound = "v10"
}
else {
    # Try to find any version
    $allVersions = Get-ChildItem -Path $PSScriptRoot -Filter "Cove2CWM-SetTicketsConfig*.ps1" -File -ErrorAction SilentlyContinue | Sort-Object Name -Descending
    if ($allVersions.Count -gt 0) {
        $Script:HelperScripts.SetTicketsConfig.Found = $true
        $Script:HelperScripts.SetTicketsConfig.Path = $allVersions[0].FullName
        if ($allVersions[0].Name -match '\.v(\d+)\.ps1$') {
            $Script:HelperScripts.SetTicketsConfig.VersionFound = "v$($matches[1])"
        }
        else {
            $Script:HelperScripts.SetTicketsConfig.VersionFound = "unknown"
        }
    }
}

# Display inventory
foreach ($scriptKey in $Script:HelperScripts.Keys | Sort-Object) {
    $helper = $Script:HelperScripts[$scriptKey]
    
    if ($helper.Found) {
        Write-Host "  ✅ $($helper.Name)" -ForegroundColor Green -NoNewline
        if ($helper.VersionFound -ne $helper.PreferredVersion) {
            Write-Host " ($($helper.VersionFound) - older version)" -ForegroundColor Yellow
        }
        else {
            Write-Host " ($($helper.VersionFound))" -ForegroundColor White
        }
        Write-Host "     Purpose: $($helper.Purpose)" -ForegroundColor Gray
        Write-Host "     Path: $(Split-Path $helper.Path -Leaf)" -ForegroundColor DarkGray
    }
    else {
        Write-Host "  ⚠️  $($helper.Name) - NOT FOUND" -ForegroundColor Yellow
        Write-Host "     Purpose: $($helper.Purpose)" -ForegroundColor Gray
        Write-Host "     Impact: $($helper.RequiredFor) will not be available" -ForegroundColor DarkYellow
    }
    Write-Host ""
}

# Summary and warnings
$foundCount = ($Script:HelperScripts.Values | Where-Object { $_.Found }).Count
$totalCount = $Script:HelperScripts.Count

if ($foundCount -eq $totalCount) {
    Write-Host "  ✅ All helper scripts found - Full functionality available" -ForegroundColor Green
}
elseif ($foundCount -eq 0) {
    Write-Host "  ⚠️  No helper scripts found - Limited functionality" -ForegroundColor Yellow
    Write-Host "     Main script will work but some features require manual configuration" -ForegroundColor DarkYellow
}
else {
    Write-Host "  ℹ️  $foundCount of $totalCount helper scripts found - Partial functionality" -ForegroundColor Cyan
}

Write-Host ""

#endregion ----- Helper Scripts Inventory ----
...
# Initialize Script:PartnerName from parameter (variable cleanup above set it to $null)
if (-not [string]::IsNullOrWhiteSpace($PartnerName)) {
    $Script:PartnerName         = $PartnerName
...
#region ----- Environment, Variables, Names and Paths ----
    ...
    if ($MonitorSystems) {
        ...
        Write-Host "    - Workstations WITH Documents Profile ($DocumentsProfileId): $StaleHoursDocuments hours" -ForegroundColor Green
        Write-Host "    - Workstations WITHOUT Documents Profile         : $StaleHoursWorkstations hours" -ForegroundColor Gray
    } else {
        Write-Host "  Servers and Workstations         : " -ForegroundColor White -NoNewline
...
Function Send-APICredentialsCookie {
    ...
    try {
        ...
        if ($Script:Authenticate.visa) {
            ...
            Write-Output "    Partner: $Script:cred0"
            Write-Output "    API URL: https://api.backup.management/jsonapi"
            Write-Output "    Credentials: $Script:APIcredfile" -ForegroundColor Gray
        } else {
            Write-Output    $Script:strLineSeparator
...
Function Send-GetDevices {
    param(
        ...
        [Parameter(Mandatory=$false)][int]$StaleHoursDocuments = 96,    # Used for workstations with Documents profile
    ...
    if ($AccountType -eq 1) {
        ...
        $failureConditions = @(
            ...
            "(OT == 1 AND OI == $DocumentsProfileId AND TL < $StaleHoursDocuments.hour().ago())",
            "(OT == 1 AND OI != $DocumentsProfileId AND TL < $StaleHoursWorkstations.hour().ago())"
        )
        # Conditionally add MSSQL-specific stale check (uses ZL = MSSQL Last Success, not device-level TL)
        if ($StaleHoursMSSQL -gt 0) {
            $failureConditions += "(I78 =~ '*D03*' AND ZL < $StaleHoursMSSQL.hour().ago())"
        }
    ...
    Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host "  Advanced Search filter: $($data.params.query.Filter)" -ForegroundColor Yellow
    ...
    • Stale Thresholds:" -ForegroundColor White
        ...
        Write-Host "      - Workstations WITH Documents Profile ($DocumentsProfileId): > $StaleHoursDocuments hours since last backup" -ForegroundColor Green
        Write-Host "      - Workstations WITHOUT Documents Profile: > $StaleHoursWorkstations hours since last backup" -ForegroundColor Gray
    } else {
        Write-Host "
    ...
    ForEach ( $DeviceResult in $SortedDevices ) {
        ...
        # Determine appropriate stale threshold based on device type (server vs workstation vs documents workstation)
        ...
        if ($AccountType -eq 1) {
            ...
            if ($isDeviceServer) {
                $DeviceStaleHours = $StaleHoursServers
            } else {
                # For workstations, check if it has Documents profile (OI column = ProfileID)
                $DeviceProfileId = [int]($DeviceResult.Settings.OI -join '')
                if ($DeviceProfileId -eq $DocumentsProfileId) {
                    $DeviceStaleHours = $StaleHoursDocuments
                } else {
                    $DeviceStaleHours = $StaleHoursWorkstations
                }
            }
        }
            ...
            if ($AccountType -eq 2) {
                ...
                foreach ($dsName in $systemDatasources.Keys) {
                    ...
                    if ($dsLastSuccess) {
                        ...
                        if ($dsHoursSinceSuccess -ge $DeviceStaleHours) {
...
Function Prompt-CWMAPICreds {
    ...
    $serverInput = Read-Host "Enter the DNS Name of your ConnectWise Manage server (e.g., staging.connectwisedev.com)"
    # Clean up protocol if user included it (prevent https://https:// URLs)
    # Also remove trailing slashes to prevent double-slash in URLs
    $CWMAPICreds.Server = ($serverInput -replace '^https?://', '').TrimEnd('/')
...
Function Get-CWMAPICreds {
    if (Test-Path $CWMAPICredsFile) {
        ...
        $secureClientId.Dispose()
        
        # Clean up protocol prefix and trailing slashes if present in stored credentials (defensive - handles legacy XML files)
        $CWMAPICreds.Server = ($CWMAPICreds.Server -replace '^https?://', '').TrimEnd('/')
...
Function AuthenticateCWM {
    ...
    # CRITICAL: PowerShell 7+ blocks HTTP↔HTTPS redirects by default
    # ConnectWise servers sometimes return redirect responses after successful API calls
    # This is safe because:
    #   1. We're connecting to a trusted ConnectWise server
    #   2. Initial connection uses HTTPS
    #   3. Module validates authentication
    # Without this, ticket operations fail with "Cannot follow an insecure redirection"
    if ($PSVersionTable.PSVersion.Major -ge 7) {
        $PSDefaultParameterValues['Invoke-RestMethod:AllowInsecureRedirect'] = $true
        $PSDefaultParameterValues['Invoke-WebRequest:AllowInsecureRedirect'] = $true
        Write-Host "  [INFO] Insecure redirects allowed for PowerShell 7+ (required for ConnectWise API)" -ForegroundColor Cyan
    }
    ...
    try {
        ...
        Write-Host "    API URL: $($CWMAPICreds.Server)" -ForegroundColor White
        Write-Host "    Credentials: $Script:CWMAPICredsFile" -ForegroundColor Gray
    } catch {
    ...
    if ($debugCWM) {
        ...
        Write-Output "Public Key: $($CWMAPICreds.pubKey.Substring(0, [Math]::Min(4, $CWMAPICreds.pubKey.Length)))****"
        Write-Output "Private Key: ****  (masked for security)"
...
Function Get-CWMCompanyForDevice {
    ...
    if ($AutoCreateCompanies -and $Device.EndCustomer -and $Device.EndCustomerLevel -eq 'EndCustomer') {
        ...
        try {
            ...
            $syncResult = & $syncScriptPath -PartnerName $Script:PartnerName -CreateCompany $matchName -NonInteractive $true -ErrorAction Stop
    ...
    # FALLBACK: Check if TicketCompany parameter was specified
    # This only activates if all 4 lookup strategies + auto-create (if enabled) failed to find a company
    if (-not [string]::IsNullOrWhiteSpace($TicketCompany)) {
        Write-Host "  No company match found - attempting fallback to TicketCompany parameter..." -ForegroundColor Yellow
        
        try {
            # Try to query CWM for TicketCompany - support both name and ID formats
            $fallbackCompany = $null
            
            # Check if TicketCompany is numeric (company ID)
            if ($TicketCompany -match '^\d+$') {
                # Query by ID
                $fallbackCompany = Get-CWMCompany -id ([int]$TicketCompany) -ErrorAction Stop
                if ($debugCWM) {
                    Write-Host "    Fallback company found by ID $TicketCompany`: $($fallbackCompany.name)" -ForegroundColor Green
                }
            } else {
                # Query by name (exact match)
                $condition = "name = `"$TicketCompany`""
                $fallbackCompany = Get-CWMCompany -condition $condition -ErrorAction Stop | Select-Object -First 1
                if ($debugCWM -and $fallbackCompany) {
                    Write-Host "    Fallback company found by name '$TicketCompany': ID $($fallbackCompany.id)" -ForegroundColor Green
                }
            }
            
            if ($fallbackCompany) {
                # Update EndCustomer cache with fallback company
                if ($Device.EndCustomer) {
                    $Script:EndCustomerToCWMCompanyCache[$Device.EndCustomer] = $fallbackCompany
                    $Script:EndCustomerCacheStats.UniqueCustomersCached.Add($Device.EndCustomer) | Out-Null
                }
                
                # Update session cache with fallback company
                $cacheKey = "$($Device.EndCustomer)|$($Device.PartnerName)"
                $Script:CompanyLookupCache[$cacheKey] = $fallbackCompany
                
                # Increment performance counter
                $Script:PerformanceMetrics.CompanyLookupCount++
                
                Write-Host "  ⚠️  FALLBACK USED: Company '$($fallbackCompany.name)' (ID: $($fallbackCompany.id)) assigned from TicketCompany parameter" -ForegroundColor Yellow
                Write-Host "     Device: $($Device.DeviceName) | EndCustomer: $($Device.EndCustomer)" -ForegroundColor DarkYellow
                
                return $fallbackCompany
            } else {
                Write-Host "  ❌ Fallback failed - TicketCompany '$TicketCompany' not found in ConnectWise" -ForegroundColor Red
            }
        }
        catch {
            Write-Host "  ❌ Error querying fallback company: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
...
Function Get-CWMTicketForDevice {
    ...
    # Search for open tickets with this device AccountId in the summary
    # Format: "COVE [DOC|365|SVR|WKS] DeviceName #AccountId - ..."
    # Search by AccountId (device ID) for 100% certain matching - avoids device name collisions
    # Exclude tickets already in closed status to prevent re-closure attempts
    $ticketFilter = "summary contains `"#$AccountId`" and closedFlag = false and board/name = `"$TicketBoard`" and status/name != `"$TicketClosedStatus`""
    
    if ($CWMCompany) {
        $ticketFilter += " and company/id = $($CWMCompany.id)"
    }
    
    # Retry logic: transport-level drops (e.g. "forcibly closed") are transient - retry before falling through to ticket CREATE
    $maxRetries = 2
    $retryDelaySec = 3
    ...
    for ($attempt = 1; $attempt -le $maxRetries; $attempt++) {
        try {
            ...
            $isTransportError = $_.Exception.Message -match 'forcibly closed|transport|connection|Unable to read data'
            if ($attempt -lt $maxRetries -and $isTransportError) {
                Write-Warning "  Ticket search failed (attempt $attempt/$maxRetries) - retrying in ${retryDelaySec}s... [$($_.Exception.Message -replace "`n",' ')]"
                Start-Sleep -Seconds $retryDelaySec
            } else {
                Write-Warning "Error searching for existing tickets (attempt $attempt/$maxRetries): $_"
                return $null  # give up - do NOT fall through to create
            }
        }
    }
    ...
    if ($existingTickets) {
        if ($debugCWM) {
            Write-Host "  Found $($existingTickets.Count) existing open ticket(s) for AccountId #$AccountId" -ForegroundColor Cyan
        }
        
        # Filter to exact AccountId match in summary (guaranteed unique match)
        # Match format: "COVE [DOC|365|SVR|WKS] DeviceName #AccountId - ..."
        # Using AccountId provides 100% certain matching - device IDs are globally unique and never change
        $exactMatches = $existingTickets | Where-Object { $_.summary -match "#$AccountId\s+-" }
        
        if ($exactMatches) {
            if ($debugCWM) {
                Write-Host "  Found exact match: ticket #$($exactMatches[0].id)" -ForegroundColor Green
            }
            return $exactMatches[0]  # Return first exact match (should only be one)
        } else {
            # No exact match found - this shouldn't happen if search filter worked
            if ($debugCWM) {
                Write-Host "  No exact match found for AccountId #$AccountId - no ticket to close/update" -ForegroundColor Gray
            }
            return $null
        }
    }
...
"@
    ...
    # Determine if this is a workstation with Documents Profile
    $isDocumentsWorkstation = ($Device.OSType -ne "2") -and ($Device.AccountType -ne 2) -and ($Device.ProfileID -eq $DocumentsProfileId)
    
    # Build ticket summary (Option D format - datasource-first)
    # Format: "COVE WKS [name] #AccountId - DS Failed - Error message" or "COVE DOC [name] #AccountId - DS Failed - Error message"
    # Use DeviceName (AN) or ComputerName (MN) based on parameter
    $subjectName = if ($UseDeviceNameInSubject) { $Device.DeviceName } else { $Device.ComputerName }
    
    if ($isDocumentsWorkstation) {
        $summary = "COVE DOC $subjectName #$($Device.AccountId) - $abbreviatedIssue"
    } else {
        $summary = "COVE $deviceTypeLabel $subjectName #$($Device.AccountId) - $abbreviatedIssue"
    }
...
$(if ($Device.Reference) { "Reference            : $($Device.Reference)`n" })$(if ($Device.Notes) { "Notes                : $($Device.Notes)`n" })Severity             : $($Device.IssueSeverity)
Description          : $($Device.IssueDescription)
Ticket Criteria      : Lookback < $DaysBack days | Stale > $(if ($Device.OSType -eq '2' -or $Device.OS -match 'Server') { "$StaleHoursServers" } elseif ($Device.ProfileID -eq $DocumentsProfileId) { "$StaleHoursDocuments" } else { "$StaleHoursWorkstations" }) hours
Creation Date        : $creationDateFormatted $creationDateRelative
Last Timestamp       : $lastTimestampDisplay $lastTimestampRelative
$(if ($oldestProblemRow) { "$oldestProblemRow`n" })
─────────────────────────────────────────────────────────────────
...
Function Update-CWMTicketForDevice {
    ...
    # Check if this is a Documents profile workstation (need to use "COVE DOC" prefix instead of "COVE WKS")
    $isDocumentsWorkstation = ($Device.OSType -ne "2") -and ($Device.AccountType -ne 2) -and ($Device.ProfileID -eq $DocumentsProfileId)
    ...
    if ($isDocumentsWorkstation) {
        $newSummary = "COVE DOC $subjectName #$($Device.AccountId) - $abbreviatedIssue"
    } else {
        $newSummary = "COVE $deviceTypeLabel $subjectName #$($Device.AccountId) - $abbreviatedIssue"
    }
    # Truncate to 100 chars max (ConnectWise limit)
...
Description          : $($Device.IssueDescription)
Ticket Criteria      : Lookback < $DaysBack days | Stale > $(if ($Device.AccountType -eq '2') { "$StaleHoursM365" } elseif ($Device.OSType -eq '2' -or $Device.OS -match 'Server') { "$StaleHoursServers" } elseif ($Device.ProfileID -eq $DocumentsProfileId) { "$StaleHoursDocuments" } else { "$StaleHoursWorkstations" }) hours
Last Timestamp       : $($Device.TimeStamp)
$(if ($oldestProblemRow) { "$oldestProblemRow`n" })
─────────────────────────────────────────────────────────────────
DATASOURCE DETAILS
...
foreach ($partner in $Script:SelectedPartners) {
    ...
    if ($MonitorSystems) {
        ...
        Send-GetDevices -AccountType 1 -StaleHours $StaleHoursServers -StaleHoursServers $StaleHoursServers -StaleHoursWorkstations $StaleHoursWorkstations -StaleHoursDocuments $StaleHoursDocuments -LookbackDays $DaysBack
    }
    ...
    foreach ($device in $DevicesWithIssues) {
        ...
        if ($CWMCompany) {
            # Search for existing ticket using indexed query (search by AccountId for 100% certain matching)
            $ticketSearchStart = Get-Date
            $existingTicket = Get-CWMTicketForDevice -AccountId $device.AccountId -CWMCompany $CWMCompany -IssueSeverity $device.IssueSeverity
...
"@
    ...
    if ($deviceIdsFromTickets.Count -gt 0) {
        ...
        try {
            ...
            $data.params.query.Columns = @('AU','AN','TL','T0','I78','AT','OT','OI','TZ',
    ...
    foreach ($ticket in $openCoveTickets) {
        ...
        # Note: We DO NOT skip tickets created/updated in this run if they should close
        # (Device is successful and not stale). Only skip if still has issues.
        # Will evaluate the device status first before deciding to skip.
        ...
        $profileId = [int]($coveDevice.Settings.OI -join '')
        
        if ($debugCDP) {
            Write-Host "[DEBUG] Ticket #$($ticket.id) for '$deviceName' - AccountType: $accountType, OSType: $osType, ProfileID: $profileId, DocumentsProfileId param: $DocumentsProfileId" -ForegroundColor Magenta
        }
        ...
        if ($lastBackupUnix -and $lastBackupUnix -ne '0') {
            try {
                ...
                # Check if backup is stale (older than threshold) - use proper server/workstation/Documents/M365 thresholds
                $osType = $coveDevice.Settings.OT -join ''
                $profileId = [int]($coveDevice.Settings.OI -join '')
                $staleThreshold = if ($accountType -eq '2') { 
                    ...
                    $StaleHoursServers 
                } elseif ($profileId -eq $DocumentsProfileId) {
                    $StaleHoursDocuments
                } else { 
                    $StaleHoursWorkstations 
                }
                ...
                if ($debugCDP) {
                    Write-Host "[DEBUG] Ticket #$($ticket.id) for '$deviceName': Last backup $([Math]::Round($hoursSinceBackup,1))h ago, Threshold: $staleThreshold h (profile=$profileId, isDoc=$($profileId -eq $DocumentsProfileId)), Stale: $isStale" -ForegroundColor Magenta
                }
            } catch {
                if ($debugCDP) {
        ...
        if ($lastSessionStatus -eq '5' -and -not $isStale) {
            # Device is healthy - CLOSE ticket even if we just created it
            ...
            # Skip first-pass tickets (created/updated in this run with issues)
            # They were created for a reason - don't process them again
            if ($ticket.id -in $justProcessedTicketIds) {
                $ticketsSkipped_JustCreatedOrUpdated++
                if ($debugCDP) {
                    Write-Host "[DEBUG] Ticket #$($ticket.id): Just created/updated with issues - skipping, will monitor on next run" -ForegroundColor Cyan
                }
                continue
            }
