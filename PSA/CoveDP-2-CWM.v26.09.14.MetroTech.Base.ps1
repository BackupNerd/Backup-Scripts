...
Param (
    ...
    [Parameter(Mandatory=$False)][Alias("PartnerName")][String]$PartnerNameOverride = "Metrotech",    ## Optional partner override used for Send-GetPartnerInfo when cred partner is too high-level
    [Parameter(Mandatory=$False)][int]$DeviceCount = 500,                                    ## Change Maximum Number of backup device results to return
...
if ($GetCDPUsage) {
    ...
    foreach ($customer in $Usage) {
        ...
        $CDPusage | Select-Object   SubDisti,Reseller,SO,EndCustomer,PartnerID,Partnername,LegalName,PartnerRef,
                                    $(if ($CombineServers) { 'CombinedServerQty' }),PhysicalServerQty,VirtualServerQty,WorkstationQty,DocumentsQty,RecoveryTestingQty,O365UsersQty,@{n='CoveGBSelected';e={[int]$_.CoveGBSelected}} |
