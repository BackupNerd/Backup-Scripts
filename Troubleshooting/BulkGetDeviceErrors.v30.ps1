...
#region ----- Authentication ----
    ...
    Function Get-APICredentials {
        ...
        $Script:APIcredfile = join-path -Path $Taskpath -ChildPath "$env:computer.\ name $env:username API_Credentials.Secure.txt"
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function GetDeviceErrors($DeviceId) {
        ...
        $Script:response = Invoke-RestMethod @params  -ErrorAction SilentlyContinue

        Start-Sleep -Milliseconds 200

        if ($Script:response.error) {Send-APICredentialsCookie}

        If ($script:response.HasItemsWithCount -eq $False) {
            ...
            Write-output "  DeviceId# $DeviceId - Skipping, initial backup in progress"
                ...
                #[array]$response1 = $Script:response.replace('true,','"true",').replace('[ESCAPE[','').replace("]]","").replace("(","\(").replace(")","\)").replace("\","\\").replace("&quot;","").replace(",`n        `n    ]","`n     ]").replace("\\(s\\)","") | ConvertFrom-Json -ErrorAction SilentlyContinue
                
                [array]$response1 = $Script:response.replace('true,','"true",').replace('[ESCAPE[','').replace("]]","").replace("\","\\").replace("\","\\").replace("&quot;","").replace(",`n        `n    ]","`n     ]").replace("(","").replace(")","") | ConvertFrom-Json -ErrorAction SilentlyContinue

                <# 
                $response1 = $response.replace('[ESCAPE[','') 
                $response2 = $response1.replace("]]","") 
                $response3 = $response2.replace("\","\\")
                $response4 = $response3.replace("&quot;","")  
                $response5 = $response4.replace(",`n        `n    ]","`n     ]") | ConvertFrom-Json -ErrorAction SilentlyContinue
                #>
            ...
            [array]$Script:cleanresponse = $response1.collection
    ...
    Function Send-UpdateCustomColumn($DeviceId,$ColumnId,$Message) {
        ...
        if ($script:updateCC.error.message) {    
            write-output "$deviceid | $Message" | out-file c:\data\errormessagestofix.txt -append
            $script:updateCC.error.message
        }

    }

    Function Send-UpdateCustomColumns($DeviceId,$ColumnId,$Message) {

        [array]$array1 = @($ColumnId,$message) | convertto-json -Depth 5
        #[array]$array2 = ,@($array1)
        ...
        $data.params = @{}
        $data.params.accountId = $DeviceId
        $data.params.values = @($array1)
        ...
        $webrequest = Invoke-WebRequest -Method POST `
            ...
            $Script:cookies = $websession.Cookies.GetCookies($url)
            $Script:websession = $websession
            $Script:updateCC = $webrequest | convertfrom-json
    ...
    Function Send-GetErrorDevices {
        ...
        Switch ($Action) {
            ...
            'Mac'{
                $DeviceFilter = "OT == 1 AND MF =~ '*apple*' AND (T7 >= 1) AND ( TS > $days.days().ago())"
            } ## Mac Workstations with Active
            'Custom'{
                #$DeviceFilter = "OT == 1 AND ( ANY =~ '*macos*' OR ANY =~ '*os X*')  AND (T7 >= 1) AND ( TS > 7.days().ago())"
                $DeviceFilter = "OT == 2 AND (T0 == 8 OR T0 == 12 OR T0 == 2 OR T0 == 3 OR T0 == 6 OR T0 == 7 OR T0 == 10 OR T0 == 11 OR T0 == 13) AND (T7 >= 1) AND ( TS > $days.days().ago())"
            } ## Mac Workstations with Active
        ...
        if (($CustomColumn) -and ($cleanresponse.message -like "*restarted*")) {
            ...
            Send-UpdateCustomColumn $DeviceResult.AccountId ($Columncode.replace("AA","")) ("Operation not permitted - Doublecheck Mac Full Disk Access " + $cleanresponse[-1].time)
            #$AbortedMSG
            ...
            #$AbortedMSG
        }elseif (($CustomColumn) -and ($cleanresponse.message -like "*Graph API request failed*")) {
            Send-UpdateCustomColumn $DeviceResult.AccountId ($Columncode.replace("AA","")) ("Graph API request failed: MailboxNotEnabledForRESTAPI.The mailbox is either inactive, soft-deleted, or is hosted on-premise. " + $cleanresponse[-1].time)
            #$AbortedMSG
        }elseif (($CustomColumn) -and ($cleanresponse.message -eq $null)) {
            Send-UpdateCustomColumn $DeviceResult.AccountId ($Columncode.replace("AA","")) ("")
            #$AbortedMSG    
        }elseif ($CustomColumn) {
