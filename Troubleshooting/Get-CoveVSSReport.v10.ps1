...
function Get-CoveDevices {
    ...
    $resp = Invoke-CoveAPI -Method "EnumerateAccountStatistics" -Visa $Visa -Params @{
        query = @{
            ...
            Columns            = @("AU", "AN", "AR", "MN", "T0", "OS", "OT", "IP", "VN", "PD", "PN", "OI", "OP", "RU", "MF", "MO", "I84", "I85", "CD", "TS", "AT")
            Filter             = if ($DeviceNameFilter) { "AN == '$DeviceNameFilter'" } else { $null }
            OrderBy            = "TS DESC"
            StartRecordNumber  = 0
            RecordsCount       = 5000
        }
...
function Analyze-Device {
    ...
    if (-not $hasData) {
        ...
        # (handles abbreviations like "PDT", non-English names, etc.)
        $deviceUtcOffsetH = 0.0
...
function Get-TlSnapRowHtml([array]$snaps, [datetime]$winStart, [double]$winSec, [int]$bwPx = 8400, [double]$axisShiftH = 0.0, [string]$TzLabel = '') {
    ...
    $ticks = ($inWin | ForEach-Object {
        ...
        $tipObj = @{
            lbl      = if ($isActive) { "⚠ ACTIVE JOB: $lbl" } else { $lbl }
...
function New-AISearchLink {
    ...
    return "<a href='$url' target='_blank' style='margin-left:6px;color:#a29bfe;text-decoration:none;cursor:pointer;font-size:0.9em' title='$Title'>✨</a>"
}
...
"@
    ...
    foreach ($w in ($Writers | Sort-Object { $_.name })) {
        ...
        $writerRows += "<tr data-wtip='$tipJson' onmouseenter='_wrShowTip(event,this)' onmousemove='_tlMoveTip(event)' onmouseleave='_tlHideTip()' style='cursor:help'>
          ...
          <td style='white-space:nowrap'><span style='color:$wrStatusColor;font-weight:$wrStatusWeight'>$($w.status)</span>$errInline$writerAiLink</td>
...
function Get-RemediationBlock {
    ...
    if ($showS1Tab) {
        ...
        if ($S1RollbackDays -gt 0 -and $S1RollbackDays -lt 7) {
            $rollbackNote = "<div style='color:#e74c3c;margin-bottom:10px'>&#9888; Rollback window: $([math]::Round($S1RollbackDays,1))d — below 7d target. Reducing snapshot interval and/or retention days will increase snapshot frequency and extend rollback coverage.</div>"
...
"@
        ...
        $cVol      = $Volumes | Where-Object { $_.drive -match '^C:?$' } | Select-Object -First 1
        $otherVols = @($Volumes | Where-Object { $_.drive -notmatch '^C:?$' -and $_.disk_total_gb -gt 50 } | Sort-Object disk_total_gb -Descending)
...
function Get-DeviceSection {
    ...
    $custHtml   = if ($D.customer_name) { "<span style=`"color:#aaa;font-size:0.75em;font-weight:normal`"> | <span class='priv'>$($D.customer_name)</span></span>" } else { "" }
    ...
    $shBadgeHtml  = if ($isWin) { "<span class='di' title='$shTip'><span class='dt $shDot'></span>$shTxt</span><span style='color:#2a2a2a'>|</span>" } else { '' }
    $wrBadgeHtml  = if ($isWin) { "<span class='di' title='$wrTip'><span class='dt $wrDot'></span>$wrTxt</span>" } else { '' }
    $badgesHtml = "<div class='bdg'>$s1BadgeHtml$rbBadgeHtml$shBadgeHtml<span class='di' title='$bkpTip'><span class='dt $bkpDot'></span>$bkpTxt</span><span style='color:#2a2a2a'>|</span>$scBadgeHtml$wrBadgeHtml</div>"
    ...
    } elseif ($D.last_timestamp) {
        $tsAge = [math]::Round(([DateTimeOffset]::UtcNow.ToUnixTimeSeconds() - $D.last_timestamp) / 3600, 0)
        $tsColor = if ($tsAge -le 168) { '#6ecf6e' } else { '#e67e22' }
        " &nbsp;|&nbsp; Last contact: <span style='color:$tsColor'>$([System.DateTimeOffset]::FromUnixTimeSeconds($D.last_timestamp).LocalDateTime.ToString('yyyy-MM-dd HH:mm'))</span>"
    } else { '' }
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
    <div>
      <h2 style="margin:0;font-size:1.3em">
        <span class="priv">$($D.machine_name)</span>
        <span style="color:#888;font-size:0.75em;font-weight:normal">(<span class="priv">$($D.device_name)</span>)</span>$custHtml
...
function Build-HtmlReport {
    ...
    for ($i = 0; $i -lt $Devices.Count; $i++) {
        ...
        if ($cust -ne $lastCust) {
            $navItems += "<div data-nav-group=`"$custKey`" style=`"padding:10px 10px 6px 10px;color:#888;font-size:0.72em;font-weight:bold;text-transform:uppercase;letter-spacing:0.05em;background:#161616;border-bottom:1px solid #2a2a2a;margin-top:0`"><span class='priv'>$cust</span></div>"
            $lastCust = $cust
        }
        $navDevType = switch ([int]($d.device_type ?? 0)) { 1 { 'workstation' } 2 { 'server' } default { 'other' } }
        $navItems += "<a href=`"#device-$i`" data-nav-sev=`"$($d.severity)`" data-nav-backup-status=`"$($d.backup_status)`" data-nav-type=`"$navDevType`" data-nav-group=`"$custKey`" data-nav-search=`"$(($d.machine_name ?? '').ToLower()) $(($d.device_name ?? '').ToLower()) $(($d.customer_name ?? '').ToLower())`" style=`"display:flex;flex-direction:column;gap:2px;padding:6px 10px 6px 18px;color:$c;text-decoration:none;border-bottom:1px solid #222;font-size:0.88em`"><span style=`"display:inline`">$icon <span class='priv'>$($d.machine_name)</span></span><span style=`"color:#666;font-size:0.8em`">$($d.t0_label)</span></a>"
    }
    ...
    if ($osStats.Count -gt 0) {
        ...
        if ($windows.Count -eq 0) {
            ...
            $windows.GetEnumerator() | Sort-Object Value -Descending | ForEach-Object {
                ...
                $fleetSoftwareHtml += "<div style='margin-bottom:2px'>$($_.Key) <span style='color:#888;font-size:0.9em'>$($_.Value) ($pct%)</span></div>"
            }
        }
