<# ----- About: ----
    # Cove Bulk Generate Redeploy Commands
    # Revision v34 - 2022-12-05 
...
# (Call the JSON Web Request Function to get the GetPassphrase Object)
        ...
        If (($d.OSVersion -match "Windows") -and ($GetPassphraseSession.error.message)) {
        Write-output ("$partnername`t$ostype`t$osversion`t$creation`t$lastsuccess`t$timestamp`t$id`t$computername`t$devicename`t$IsPassphrase`t$password`tUsing Private Encryption Key`t$Wincmddownload;" + '& C:\Users\Public\Downloads\' + "mxb-windows-x86_x64.exe -silent -user `"$DeviceName`" -password `"$password`" -encryption-key `"INSERT_PRIVATE_KEY`"`t$WinLegacydownload;" + '& C:\Users\Public\Downloads\' + "mxb-windows-x86_x64.exe -silent -user `"$DeviceName`" -password `"$password`" -encryption-key `"INSERT_PRIVATE_KEY`"") | Out-file $csvcommands -append

        }elseif (($d.OSVersion -eq "") -and ($passphrase -ne $null)) {
            Write-output ("$partnername`t$ostype`t$osversion`t$creation`t$lastsuccess`t$timestamp`t$id`t$computername`t$devicename`t$IsPassphrase`t$password`t$passphrase`t$Wincmddownload;" + '& C:\Users\Public\Downloads\' + "mxb-windows-x86_x64.exe -silent -user `"$DeviceName`" -password `"$password`" -passphrase `"$passphrase`" -restore-only `t$WinLegacydownload;" + '& C:\Users\Public\Downloads\' + "mxb-windows-x86_x64.exe -silent -user `"$DeviceName`" -password `"$password`" -passphrase `"$passphrase`" -restore-only") | Out-file $csvcommands -append

        }elseif (($d.OSVersion -match "Windows") -and ($passphrase -ne $null)) {
            Write-output ("$partnername`t$ostype`t$osversion`t$creation`t$lastsuccess`t$timestamp`t$id`t$computername`t$devicename`t$IsPassphrase`t$password`t$passphrase`t$Wincmddownload;" + '& C:\Users\Public\Downloads\' + "mxb-windows-x86_x64.exe -silent -user `"$DeviceName`" -password `"$password`" -passphrase `"$passphrase`" -restore-only `t$WinLegacydownload;" + '& C:\Users\Public\Downloads\' + "mxb-windows-x86_x64.exe -silent -user `"$DeviceName`" -password `"$password`" -passphrase `"$passphrase`" -restore-only") | Out-file $csvcommands -append

        }elseif (($d.OSVersion -match "macos") -or ($d.OSVersion -match "OS X"))  {
            Write-output ("$partnername`t$ostype`t$osversion`t$creation`t$lastsuccess`t$timestamp`t$id`t$computername`t$devicename`t$IsPassphrase`t$password`t$passphrase`t$MACdownload`t$macinstaller") | Out-file $csvcommands -append

        }elseif (($d.OSVersion -notmatch "macos") -and ($d.OSVersion -notmatch "OS X") -and ($d.OSVersion -notmatch "Windows")) {
            Write-output ("$partnername`t$ostype`t$osversion`t$creation`t$lastsuccess`t$timestamp`t$id`t$computername`t$devicename`t$IsPassphrase`t$password`t$passphrase`tManual`tManual") | Out-file $csvcommands -append
