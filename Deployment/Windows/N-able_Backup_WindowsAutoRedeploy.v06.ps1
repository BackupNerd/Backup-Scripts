<# ----- About: ----
    # N-able Backup Windows Auto Redeploy
    # Revision v06 - 2022-04-23 
...
Function Authenticate {
    ...
    $script:AdminPassword = [Runtime.InteropServices.Marshal]::PtrToStringAuto([Runtime.InteropServices.Marshal]::SecureStringToBSTR($Credential.Password))
    ...
    # (No error)
        ...
        ForEach ( $Result in $EnumerateAccountStatisticsSession.result.result ) {
            $script:DeviceDetail += New-Object -TypeName PSObject -Property @{ AccountID    = [String]$Result.AccountId;
                                                                        ...
                                                                        ActivationKey = $Result.Settings.QW -join '' ;
...
Function Download-BackupManager {
    ...
    if ((Test-Path $BackupManager -PathType leaf) -eq $false) {
        ...
        write-output "  Downloading Backup Manager - TLS"; (New-Object System.Net.WebClient).DownloadFile($HttpsCDN,$BackupManager)
        }  ## Attempt download over TLS 1.0

    if ((Test-Path $BackupManager -PathType leaf) -eq $false) {
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls11   
        write-output "  Downloading Backup Manager - TLS 1.1"; (New-Object System.Net.WebClient).DownloadFile($HttpsCDN,$BackupManager)
        }  ## Attempt download over TLS 1.1

    if ((Test-Path $BackupManager -PathType leaf) -eq $false) {
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12   
        write-output "  Downloading Backup Manager - TLS 1.2"; (New-Object System.Net.WebClient).DownloadFile($HttpsCDN,$BackupManager)
        }  ## Attempt download over TLS 1.2
}
