...
<# ----- Behavior: ----
    ...
    # It downloads, extracts and runs Servertool sequentually against a CSV device list to build or update a set of offsite speedvault
    #
    ...
    #   Use the -NetworkPath parameter to set the base network path, otherwise a default parameter path is used
...
[CmdletBinding(DefaultParameterSetName="Network")]
    Param (
        ...
        [Parameter(ParameterSetName="Network",Mandatory=$False)] [String]$NetworkPW = "Password",                      ## Set Password for network path
        ...
        [Parameter(Mandatory=$False)][ValidateRange(1,200)][int]$threadcount = 100,                                    ## Seed download thread count
...
#region ----- Functions ----
    ...
    Function Start-SeedDownload {
        ...
        Write-output "`n  **Command** Servertool.exe seed.download -account $($device.{Device name}) -password $($device.{Installation key}) -path $($VaultStoragePath) -thread-count $($threadcount) -retry-count $($retrycount)"
...
#region ----- Main body ----    
    ...
    Switch ($PSCmdlet.ParameterSetName) { 
        'Network' {
            ...
            # Select Devices
            $script:selected = $lookup | Out-GridView -OutputMode Multiple
