...
Function Set-Alias {
    ...
    if ((Test-Path $BMConfig -PathType leaf) -eq $true) {
        ...
        $script:alias = "$(get-timestamp) Cove Prep"
...
if (((Test-Path $Script:BMConfigCopy -PathType leaf) -eq $true) -and ((Test-Path $Script:BackupIp -PathType leaf) -eq $false) -and ($Script:IsIntegrated -eq "True" ))  {
