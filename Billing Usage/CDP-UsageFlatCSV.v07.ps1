...
#
# Usage:
#   .\CDP-UsageFlatCSV.ps1                                     # picker UI
#   .\CDP-UsageFlatCSV.ps1 -creds prod4 -Timeframe 202605      # XML cred by ID
#   .\CDP-UsageFlatCSV.ps1 -SFDCID <id> -APIKey <key>          # direct creds
#
...
Param (
    ...
    [Parameter(Mandatory=$False)] [string]$Timeframe   = "202605",
...
if ($SFDCID -and $APIKey) {
    # ── Direct parameter mode: SFDCID + APIKey supplied at command line ──────────
    $Token   = $APIKey
    $Account = if ($AccountName) { $AccountName } else { "SFDCID-$SFDCID" }
    Write-Output "Using direct credentials (SFDCID: $SFDCID)"
} else {
    # ── XML credential store mode (default) ──────────────────────────────────────
    $credsFile = Join-Path $PSScriptRoot 'BillingCreds.xml'
    if (-not (Test-Path $credsFile)) {
        Write-Warning "BillingCreds.xml not found at: $credsFile"
        Write-Warning "Either create BillingCreds.xml or supply -SFDCID and -APIKey directly."
        exit 1
    }
    $allCreds = Import-Clixml $credsFile

    if (-not $creds) {
        # No account specified — show account name picker
